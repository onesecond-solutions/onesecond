...
### 2.6 결정 항목 신규 후보 (5~7건 — J-1 ~ J-7) — ✅ **2026-05-04 일괄 결재 완료 (옵션 I 권장값 7건)**

| # | 결정 후보 | 채택 | 옵션 (참고) |
|:--:|---|---|---|
| **J-1** | 모더레이션 액션 표준 | ✅ **(a) 3종 — 숨김 + 삭제 + 정지** | (b) 숨김+삭제만 / (c) 숨김만 |
| **J-2** ⭐ | post_reports 테이블 처리 | ✅ **(b) v2.0 대기 + mock 라벨만 동적** — DB 변경 0건, 라벨 `[Phase C mock]` → `[v2.0 대기]` | (a) D-3 진입 시 신설 (DB 1테이블 + RLS 4건) |
| **J-3** | 사용자 정지 액션 활성 | ✅ **(a) D-3 직접** — D-pre.5 status 컬럼 활용 | (b) D-final 보안 검증으로 이관 |
| **J-4** | 보험사 게시판 v2.0 대기 표시 | ✅ **(a) Phase C mock 보존 (회색 점선)** | (b) 차트 라인 제거 |
| **J-5** | 라인차트 데이터 소스 | ✅ **(a) 클라이언트 GROUP BY 우선** — P3·P4 FAIL 시 (b) `get_board_activity_90d` RPC 신설 별 트랙 격상 | (b) RPC 우선 |
| **J-6** | KPI 댓글 카운트 정의 | ✅ **(a) comments 전체 count** | (b) 활성 게시글의 댓글만 |
| **J-7** | 신고 자동 알림 | ✅ **(a) D-3 범위 외** | (b) Sentry/Slack hook |

**결재 영향 정합:** J-2 (b) 채택으로 DB 변경 0건 → Step 분할 5단계 표준 (D-2 패턴) + 라이브 회귀 ~25항목 + 잔여 견적 1.0~1.3세션. D-3 작업지시서 본 발행 — `docs/specs/admin_v2_d3_workorder.md`.
...
### 3.5 결정 항목 신규 후보 (4~6건 — K-1 ~ K-6) — ✅ **2026-05-04 일괄 결재 완료 (옵션 I 권장값 6건)**
...
|---|:--:|---|
| ~~D-3 board~~ | ~~1.3~~ | ✅ 종료 (5/4, 25/25 PASS, `f5c6c5e`) |
| ~~D-4 notice~~ | ~~1.0~~ | ✅ 종료 (5/4, 20/20 PASS, `27f0688` + `70fb91c`) |
| ~~D-6 logs~~ | ~~0.8~~ | ✅ 종료 (5/5, 20/20 PASS, `33b3e24` + `d72320a`) |
| ~~D-5 analytics~~ | ~~1.8~~ | ✅ **종료 (5/5, 29/30 PASS — L7 UTC 조건부, `33df2f7` + 본 회차) — RPC 4종 + B-1 토큰** |
...
| 회귀·fix 마진 (+30%) | 1.5 | D-1~D-6 실측 + D-9 마진 (D-5 종료로 마진 축소) |
| **잔여 D 합계 (5/5 D-5+D-6 종료 후)** | **~5.9세션** | (5/4 D-4 종료 시점 ~9.1 → -0.8 D-6 → -1.8 D-5 → 마진 축소 = ~5.9) |
