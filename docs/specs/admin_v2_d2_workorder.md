...
// ── stage DB 영문 슬러그 → 한국어 번역 (Step 1 사전 검증 결과 반영, F-1 청산) ──
//   2026-05-04 결정 (i) — DB 진실 원천 + 한국어 번역만. mock 한국어 라벨(1.인사·라포 등) 폐기.
//   17년 현장 분류(DB raw)가 외부 시연 신뢰도 ↑.
//   Step 1-1 (4) raw: opening/opening_rejection/situation_check/analysis/need_emphasis/
//                     need_emphasis_2/product/objection/closing/closing_second (10종, 합 59)
var STAGE_LABELS_KO = {
  opening:           '오프닝',
...
// 도넛·범례 표시 순서 (영업 단계 흐름 — opening → closing_second)
var STAGE_KEY_ORDER = [
...
async function fetchRecentContent(limit) {
  ...
  var [scriptsRes, libraryRes] = await Promise.all([
    ...
    // ⚠️ scope=neq.private — admin 본 계정 테스트 데이터(2026-05-04 발견:
    //   "임태성 자료 추가하기 제목 테스트") 시연 noise 차단. private 일반 차단으로
    //   다른 사용자 개인 자료도 D-2 콘텐츠 테이블에서 제외 (의도 정합)
    window.db.fetch(
...
function renderStageDonut(dist) {
  ...
  // STAGE_KEY_ORDER 순서대로 DB 영문 키 → 한국어 라벨 매핑 (F-1 청산 결과 반영)
  var ordered = STAGE_KEY_ORDER.map(function (key, i) {
    var cnt = dist.counts[key] || 0;
    var label = STAGE_LABELS_KO[key] || key;
    return { key: key, label: label, count: cnt, color: STAGE_COLORS[i],
...
- [ ] STAGE_LABELS_KO 10키가 DB raw 영문 슬러그(opening/opening_rejection/situation_check/need_emphasis/need_emphasis_2/analysis/product/objection/closing/closing_second)와 1:1 정합
...
| 3 | ~~🔴 격상 — `get_stage_distribution` RPC 신설~~ ✅ **함수 신설 청산 (2026-05-04 commit `788b617`)** — SECURITY DEFINER + `is_admin()` 가드 + REVOKE/GRANT 정합. **단 P3 라운드트립 <200ms 미달 (min 208ms, avg ~440ms — PostgREST overhead 본질 변동성, P4 PASS로 사용자 영향 0)** → 🟡 별 트랙 분리 (`docs/specs/admin_v2_p3_postgrest_analysis.md` Phase E 격상) | I-2 → 청산 + P3 별 트랙 |
| 4 | `owner_id` (text) ↔ `users` join 패턴 RPC (작성자 이름·역할 표시) | I-5 |
| 5 | KPI 카드 추세 라벨 "▲ 184건 vs 지난 달" 동적화 (기간 비교 RPC) | Step 3-1 |
| 6 | content 메뉴 pane 잔여 항목(.pending 유지) Phase E 범위 정합 | Step 3-7 |
| 7 | `.adm-toast` D-3~D-8 적용 (D-1에서 신설, D-2도 사용) | D-1 누적 |
| 8 | ~~STAGE_LABELS 매핑 테이블 신설~~ ✅ **청산 (2026-05-04, Step 1 사전 검증 결과 (4) raw 반영)** — § 2-1 코드에 `STAGE_LABELS_KO` 객체 + `STAGE_KEY_ORDER` 배열 명문화. 결정 (i) 채택: DB 진실 원천 + 한국어 번역. mock 한국어 라벨(1.인사·라포 등) 폐기. admin_v2.html § 단계별 비율 panel 라벨도 STAGE_KEY_ORDER 기반 동적 생성으로 정합. | scripts 보강 트랙 5/4 사전 검증 (F-1) |
| 9 | **scripts.use_count 59건 모두 0** — `script_usage_logs` 집계 → `use_count` 갱신 RPC/트리거 신설 필요 (D-pre.8 § 5.5 참고). D-2 직접 영향 0 (KPI "조회" 컬럼 매핑 정합, 표시값만 0) | D-2 Step 1 사전 검증 (5) 발견 |
| 10 | **library 1행 noise 처리 — `scope=private` 필터 적용**(D-2 fetchRecentContent에 적용) ✅ — admin 본 계정 테스트 데이터 ("임태성 자료 추가하기 제목 테스트") D-2 콘텐츠 테이블에서 차단. 별 트랙: 라이브러리에 시연용 다양한 자료 seed 추가 (현재 library_total=1로 시연 가치 낮음) | D-2 Step 1 사전 검증 (3) 발견 |
