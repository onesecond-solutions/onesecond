...
| **D-pre.6** users_role_check 9역할 정합 + D-pre 누락 영역 전수 | 🟡 **사양 확정 (2026-05-02), Step A 진행 중** | D-pre.5 Step C 발견 + role 감사 신규 발견 처리. **사양 확정**: Code 추천 5결정 전면 채택 (`docs/specs/d-pre6-spec-analysis.md` 커밋 `8ab03cf` § 7 통합 사양). 7단계 진행 (Step A 사전 검증 → B DROP+ADD 분할 → C board.html Edit → D 사후 검증 → E 라이브 5건 → F 롤백 SQL 보관 → G 단일 커밋·푸시). board.html 줄바꿈 4줄 형식. INSERT 시뮬레이션 거부(ALTER 성공+정의 raw 비교+admin row 통과로 충분). Step B+C 단일 커밋 묶음. E-5 admin이 "[D-pre.6 회귀 검증]" 표시 후 삭제 |
