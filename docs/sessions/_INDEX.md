...
> **마지막 갱신:** 2026-05-05 후속 — **트랙 #A PITR 결제 직전 Chrome 5/5 PASS** (Step A 사전 검증 + B-prep 결제 화면 도달 raw, capture `db_pitr_activation_capture.md` 신설) / 진행 순서 정정 = Compute Small 먼저 → PITR 7 days 나중 / 비용 raw 정정 = 신규 청구 ~$130.15/월 / 즉시 결제 X (prorated). 결제 후 사후 검증 의뢰서 별도 발행 대기. (직전 갱신 = v1.1 안전장치 결재 a/a/a 회신)
