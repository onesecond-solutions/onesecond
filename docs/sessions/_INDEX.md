...
> **마지막 갱신:** 2026-05-05 (D-5·D-6 완전 종료 — D-5 29/30 PASS + RPC 5종 신설 + B-1 grid 토큰 5종 톤 + D-6 20/20 PASS + admin_read_all_logs is_admin() 청산 + v1.1 운영 안전장치 3종 일정 명문화 결정 문서 발행 + 잔여 견적 ~9.1 → ~6.5세션)
...
6. **(신규 5/5) v1.1 운영 안전장치 3종 결재 3건** — 결정 문서 `docs/decisions/2026-05-05_v1_1_safety_3track_schedule.md` § 5 인용:
   - **#1 PITR 비용 결재** — (a) 활성 ($100~$400/월 + Small Compute $10~$20) / (b) Daily 백업 유지 / (c) 4/24 RAM 업그레이드만. **Code 권장 (a)** — 4/22~23 사고 학습 정합
   - **#2 Sentry.io DSN 발급** — (a) 무료 플랜 (5,000 events/월) / (b) Team $26/월. **Code 권장 (a)** — 4팀 40명 충분
   - **#3 Playwright 도입 범위** — (a) 회귀 자동화 1세트 / (b) 전체 + CI / (c) Cypress 대체. **Code 권장 (a)** — 4팀 오픈 직전 가성비
...
- `docs/sessions/2026-05-05_<TBD>.md` — 2026-05-05 (admin_v2 Phase D-5·D-6 완전 종료 단일 세션 — push 9 commit / D-6 logs 4건 (`7cda0b8` 작업지시서 / `33b3e24` 코드 + admin_read_all_logs is_admin() 청산 / `21968ac` 의뢰서 / `d72320a` 20/20 PASS 종료) + 정리 1건 (`ee53976` 두 5/5 작업지시서 docs/sessions → docs/specs 이동) + 안전장치 1건 (`4cd4603` v1.1 운영 안전장치 3종 일정 명문화 결정 문서 — PITR 5/7 / Sentry 5/12 / Playwright 5/13~14) + D-5 analytics 4건 (`4957e53` 작업지시서 + L-1~L-10 결재 / `b6912f8` RPC 4종 신설 + 정합 검증 15/15 PASS / `33df2f7` 코드 js +271줄 + B-1 grid 토큰 5종 톤 + 의뢰서 30항목 / 본 commit 29/30 PASS 종료) / M-1~M-9 + L-1~L-10 결재 일괄 / D-pre.8 sweep 누락 보강 (admin_read_all_logs 청산 D-4 패턴 정합) / 영구 학습 후보 — RPC SECURITY DEFINER 5함수 일괄 + has_function_privilege 직접 조회 검증 패턴 + set_config admin 시뮬 패턴 + AT TIME ZONE 'Asia/Seoul' L-1 정합 + features CTE LEFT JOIN UX 안정 + last_seen_at 갱신 미구현 사후 발견 + UTC vs KST today 환경 차이 / 잔여 견적 ~9.1 → ~5.9세션 / D-9 ⚙️ 화면설정 즉시 진입 가능 + 별 트랙 #A PITR 5/7 진입 가능 (비용 결재 후))
...
| **#A v1.1 PITR** | Supabase 백업 시스템 강화 | Daily 백업만 가용 → 분 단위 복구 부재 | 4/22~23 사고 시점에도 Daily 백업만 가용. 비용 결재 후 Dashboard 활성. 결정 문서 `docs/decisions/2026-05-05_v1_1_safety_3track_schedule.md` § 4-A | **5/7 (목)** D-5 Step 4 병렬 진입 (0.2세션) — 결재 #1 승인 후 |
| **#B v1.1 Sentry** | 라이브 에러 추적 SDK 도입 | 코드 hook 준비됨 / 실 SDK 미도입 | DSN 발급 (Sentry.io 무료 플랜) + 초기화 코드 + 라이브 검증. 결정 문서 § 4-B | **5/12 (화)** D-5 종료 + 주말 패키지 후 진입 (0.5~0.8세션) — 결재 #2 승인 후 |
| **#C v1.1 Playwright** | 라이브 회귀 자동화 1세트 | 코드·문서 0건 / Chrome 위임 사람 의존 | admin_v2 D-1~D-6 회귀 자동화 ~50 시나리오. 결정 문서 § 4-C | **5/13~14 (수·목)** 4팀 오픈 직전 (1.0~1.5세션) — 결재 #3 승인 후 |
