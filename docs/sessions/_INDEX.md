...
| **D-pre.7** Phase D 8테이블 admin SELECT 정책 점검 → users + library 2건 (1차 EXISTS 사고 + 2차 SECURITY DEFINER 재진입) | ✅ **완료 (2026-05-02)** | D-1 Step 6 R6 검증 SQL로 발견된 admin SELECT 정책 부재 청산. 결정 #1 "분리" 정합 유지. **A 본문 정밀 검토로 6→2 정정** (qual=true 패턴은 admin 이미 가능). **🚨 1차 EXISTS 자기 참조 패턴 → PostgreSQL 무한 재귀(42P17) 발생** (라이브 검증에서 발견, DB 메타 검증은 통과 = 메타-런타임 격차) → 비상 롤백 (DROP POLICY 2건) → 라이브 회귀 검증 status=200 복구 → **2차 SECURITY DEFINER 함수 패턴 재진입** (`is_admin()` STABLE + `security_type=DEFINER` 검증) + 새 정책 2건 (USING `public.is_admin()`) → C 정의 raw 비교 (구 EXISTS 잔존 0건 + `is_admin()` 패턴 정합) + 카운트 0→1 → D 라이브 3건 status=200 + 재귀 회귀 0. **2차 9건 전건 통과 + 1차 사고 학습 영구 명문화**: (1) RLS USING/WITH CHECK에 동일 테이블 SELECT 서브쿼리 절대 금지 (2) admin/role 검증은 SECURITY DEFINER 함수 표준 (3) DB 메타 통과 ≠ 라이브 안전, 라이브 검증 필수 (4) `admin_update_all_users` 작동해도 같은 패턴 SELECT 추가 시 chain 형성으로 재귀 (5) Code 자체 "재귀 안전 ✅" 단정 결론 금지. 산출물: `docs/architecture/db_pre_dpre7_capture.md` (1차 사고 보존 + 2차 재진입 + 영구 학습). 잔존 부채: posts is_hidden + news 후순위 + RLS 자기 참조 회피 표준 메모리 등록. **D-pre 시리즈 모두 종료 → D-1 본 진입 100% 정합 보장** |
