...
> ✅ **완료 (2026-05-12 라이브 검증) / 이력 보존용 (2026-05-31 표기).**
> 본 문서는 실행 전 계획 초안이다. 9역할 마이그레이션은 2026-05-12 라이브 검증 완료됐다(CLAUDE.md "Phase 1 9역할 마이그레이션 완료 (2026-05-12 검증)" 정합, 옛 5역할 잔존 0건). 본문 헤더의 "🟡 SQL 초안 — 실행 금지"는 **작성 시점(2026-05-01) 상태 표기이며 현재는 무효**다. 본 문서의 SQL을 다시 실행하지 말 것.
