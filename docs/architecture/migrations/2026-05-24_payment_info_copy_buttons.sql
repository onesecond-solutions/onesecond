-- ════════════════════════════════════════════════════════════════════
-- 2026-05-24 payment_info 카드 우측 상단 "⚡ 딸깍" 복사 버튼 자체 추가
-- ════════════════════════════════════════════════════════════════════
-- 본 SQL 자체:
--   payment_info content_html 전체 UPDATE
--   - 직전 batch1 + 흥국화재 카드납 변경 자체 (직전 마감)
--   - 각 회사 카드 자체 자체 (23개) = 우측 상단 ⚡ 딸깍 버튼 자체 추가
--   - 카드 자체 자체 = class "pay-card" + position:relative 자체
--   - 클릭 시 자체 = window.copyPaymentCardV2(this) 자체 자체 자체 자체
--     → 카드 본문 자체 자체 = clipboard 복사 + "복사 완료. 카카오톡에 붙여넣기 하세요." 안내
--
-- 손해보험 그룹 (12 카드) = 인디고 버튼 색상
-- 생명보험 그룹 (11 카드) = 보라 #A78BFA 버튼 색상
--
