-- ════════════════════════════════════════════════════════════════════
-- 2026-05-24 mirror_script V2 토큰 정합 (인라인 스타일 → CSS 변수)
-- ════════════════════════════════════════════════════════════════════
-- 격차 본질:
--   직전 seed SQL (2026-05-24_mirror_script_seed.sql) 안 content_html =
--   인라인 스타일 자체 (#FFFDE7 / #E53935 / #1E88E5 자체) → 라이트 모드 고정
--   → V2 다크 모드 정합 X
--
-- 정정 자료:
--   인라인 색상 → CSS 변수 (var(--ac) / var(--err) / var(--info) / var(--s2) 등)
--
-- 매핑 자료:
--   #FFFDE7 (노랑 배경)    → var(--s2) (보조 배경)
--   #E3F2FD (파랑 헤더)   → var(--s2) (보조 배경)
--   #FEF3C7 (경고 배경)   → var(--s2)
--   #F4F4F6 (회색 배경)   → var(--s2)
--   #FFFFFF (흰 배경)     → var(--s1) (메인 배경)
--   #E4E4E7 (점선 회색)   → var(--bd) (테두리)
--   #E53935 (빨강 강조)   → var(--err)
--   #F59E0B (주황 강조)   → var(--warn)
--   #92400E / #78350F     → var(--warn)
--   #1E88E5 / #1976D2     → var(--ac) (인디고 강조)
--   #18181B (검정 텍스트) → var(--tp) (primary)
--   #52525B (회색 텍스트) → var(--ts) (secondary)
--   #3F3F46 (본문 회색)   → var(--bodytx)
--   #6B7280 (연회색)      → var(--tf) (faint)
--
-- 실행: Supabase SQL Editor → 통째 붙여넣기 → RUN
-- ════════════════════════════════════════════════════════════════════

BEGIN;

-- ── 1. 미러링 전 녹취 스크립트 (회사 선택 허브) ─────────────────────
UPDATE public.quick_contents
SET content_html = $HTML$
<div class="mirror-script-hub">
  <div style="background:var(--s2);padding:12px 16px;border-radius:8px;margin-bottom:14px;">
  ...
  <div style="background:var(--s2);border-left:4px solid var(--warn);padding:12px 14px;border-radius:6px;margin-bottom:14px;">
  ...
  <div style="background:var(--s2);padding:14px 16px;border-radius:8px;margin-bottom:14px;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
      <div style="font-size:0.95em;font-weight:700;color:var(--tp);">미러링 전 동의 녹취</div>
      <div style="font-size:0.75em;color:var(--tf);font-style:italic;">(핵심만)</div>
    </div>
    <p style="line-height:1.7;margin-bottom:12px;color:var(--bodytx);">
    ...
    <div style="background:var(--s1);border:1px dashed var(--bd);padding:10px 12px;border-radius:6px;font-size:0.8em;color:var(--ts);text-align:center;">
      아래 보험사를 선택하면 해당 녹취 스크립트를 바로 확인할 수 있습니다.
    </div>
  </div>

  <div style="display:flex;gap:14px;justify-content:center;margin-top:18px;">
    <button type="button" onclick="openQuickOverlay('메리츠화재')" style="flex:1;max-width:200px;padding:14px 18px;background:transparent;border:1.5px solid var(--bd);border-radius:10px;color:var(--tp);font-size:0.95em;font-weight:700;font-family:inherit;cursor:pointer;transition:all 0.15s ease;">메리츠화재</button>
    <button type="button" onclick="openQuickOverlay('흥국생명')" style="flex:1;max-width:200px;padding:14px 18px;background:transparent;border:1.5px solid var(--bd);border-radius:10px;color:var(--tp);font-size:0.95em;font-weight:700;font-family:inherit;cursor:pointer;transition:all 0.15s ease;">흥국생명</button>
  </div>
</div>
$HTML$
...
-- ── 2. 메리츠화재 (mirror_script) ───────────────────────────────────
UPDATE public.quick_contents
SET content_html = $HTML$
<div class="mirror-script">
  <div style="background:var(--s2);padding:10px 14px;border-radius:8px;text-align:center;font-weight:800;font-size:1em;margin-bottom:14px;color:var(--tp);">메리츠화재 한장으로 읽어요 (26년 4월 업데이트 자료)</div>

  <div style="background:var(--s2);padding:8px 12px;border-radius:6px;text-align:center;font-weight:800;margin-bottom:10px;color:var(--tp);">상담콜</div>

  <p style="line-height:1.7;margin-bottom:10px;color:var(--bodytx);">
  ...
  <p style="line-height:1.7;margin-bottom:14px;color:var(--bodytx);">
  ...
  <div style="background:var(--s2);padding:8px 12px;border-radius:6px;text-align:center;font-weight:800;margin-bottom:10px;color:var(--tp);">가입설계동의</div>

  <p style="line-height:1.7;margin-bottom:8px;color:var(--bodytx);">본 동의는 가입설계동의이며, 거부가능하나 정확한 상담은 불가합니다.</p>

  <p style="line-height:1.7;margin-bottom:8px;color:var(--bodytx);">
  ...
  <div style="background:var(--s2);padding:10px 12px;border-radius:6px;text-align:center;font-weight:800;color:var(--err);">
...
-- ── 3. 흥국생명 (mirror_script) ─────────────────────────────────────
UPDATE public.quick_contents
SET content_html = $HTML$
<div class="mirror-script">
  <div style="background:var(--s2);padding:10px 14px;border-radius:8px;text-align:center;font-weight:800;font-size:1em;margin-bottom:14px;color:var(--tp);">흥국생명 VTM 사전녹취스크립트 (2025.11.27 기준)</div>

  <div style="background:var(--s2);padding:8px 12px;border-radius:6px;font-weight:800;margin-bottom:10px;color:var(--tp);">모집자 소속 / 성명 / 통화목적</div>
  <p style="line-height:1.7;margin-bottom:10px;color:var(--bodytx);">
  ...
  <div style="background:var(--s2);padding:8px 12px;border-radius:6px;font-weight:800;margin-bottom:10px;color:var(--tp);">정보취득경로 (개인정보보호취득경로 18.09.01)</div>
  <p style="line-height:1.7;margin-bottom:10px;color:var(--bodytx);">
  ...
  <div style="background:var(--s2);padding:8px 12px;border-radius:6px;font-weight:800;margin-bottom:10px;color:var(--tp);">두낫콜</div>
  <p style="line-height:1.7;margin-bottom:14px;color:var(--bodytx);">원치 않으시면 통화 거절도 가능하십니다.</p>

  <div style="background:var(--s2);padding:8px 12px;border-radius:6px;font-weight:800;margin-bottom:10px;color:var(--tp);">상품설명 <span style="color:var(--err);">※ 주계약 및 특약별로 가입한 내용에 따라 보장안내</span></div>
  <p style="line-height:1.7;margin-bottom:8px;color:var(--bodytx);">
  ...
  <div style="background:var(--s2);padding:8px 12px;border-radius:6px;font-weight:800;margin-bottom:10px;color:var(--tp);">신용정보 및 계약관리자 안내</div>
  ...
  <div style="background:var(--s2);padding:8px 12px;border-radius:6px;font-weight:800;margin-bottom:10px;color:var(--tp);">청약 녹음</div>
  <p style="line-height:1.7;margin-bottom:14px;color:var(--bodytx);">
  ...
  <div style="background:var(--s2);padding:8px 12px;border-radius:6px;font-weight:800;margin-bottom:10px;color:var(--tp);">청약서 고지사항 — 계약전 알릴의무</div>
  ...
  <div style="background:var(--s2);padding:8px 12px;border-radius:6px;font-weight:800;margin-bottom:10px;color:var(--tp);">카드 납입</div>
  ...
  <p style="line-height:1.7;margin-bottom:14px;color:var(--bodytx);">승낙이 이루어진 경우 1회 보험료를 받은 때부터 효력이 발생합니다.</p>

  <div style="background:var(--s2);padding:8px 12px;border-radius:6px;font-weight:800;margin-bottom:10px;color:var(--tp);">계좌 납입</div>
  ...
  <p style="line-height:1.7;color:var(--bodytx);">승낙이 이루어진 경우 1회 보험료를 받은 때부터 효력이 발생합니다.</p>
</div>
$HTML$
