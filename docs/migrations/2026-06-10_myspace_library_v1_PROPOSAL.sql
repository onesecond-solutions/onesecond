...
-- Code 실행 안 함. 대표님이 Dashboard에서 직접 실행.
-- 2026-06-10 결재: STEP 0 실측 통과(테이블·버킷 미존재 확인) → 1번안(STEP 1~4 한 번에) 확정.
-- 순서: STEP 0(실측, 읽기·완료) → STEP 1~3b(생성) → STEP 4(RLS 즉시 적용) → 검증 SELECT.
...
-- ============================================================
-- 🟠 STEP 4. RLS / 정책 — 즉시 적용 (2026-06-10 대표님 결재: 최소 보안 기본값 즉시 적용)
-- 개인 격리: (auth.uid())::text = owner_id. 패턴 출처: scripts_delete_own_rls.sql.
-- 고도화(공유·티어 등)는 정책 확정 후 별 트랙. 재실행 안전 위해 drop policy if exists 선행.
-- ============================================================
...
drop policy if exists myspace_usage_own on public.myspace_usage;
...
-- (storage.objects 는 Supabase에서 RLS 기본 ON — 정책만 추가)
