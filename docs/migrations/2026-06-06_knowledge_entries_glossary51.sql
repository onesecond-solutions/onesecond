-- 보험 용어 기초 사전 1차 51개 (50개 + 품질보증해지 분리 추가)
...
('term','계약전알릴의무','고지의무의 다른 이름. 계약 체결 전 중요한 사항을 사실대로 알릴 의무.','의무',array['계약전알릴의무','고지의무'],'official_glossary','fss_finedic','금융감독원 금융용어사전','ai_draft','high','admin'),
...
('term','대기기간','보장이 시작되기까지 기다려야 하는, 약관에서 정한 기간. 면책기간과 비슷한 뜻으로 쓰인다.','청구',array['대기기간','감액기간'],'official_glossary','fss_finedic','금융감독원 금융용어사전','ai_draft','high','admin'),
...
-- 검증 (별도 RUN, 읽기)
-- select count(*) from public.knowledge_entries where source_type='official_glossary';  -- 51 기대
-- select status, count(*) from public.knowledge_entries group by status;  -- approved 72 + ai_draft 51
