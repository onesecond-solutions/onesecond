> ⚠️ **폐기 — 2026-05-08**
>
> **사유:** 본 spec(v1)은 2026-05-07 작성 시점 spec이며, 다음 사항이 본질·결정과 어긋나 폐기됨:
>
> 1. **현장 Q&A 쓰기 권한** — v1 = 전원 쓰기 / 본질 = 시스템 자동 정제 결과 (사용자 직접 쓰기 ❌)
> 2. **네비게이션방 부재** — v1 = 보드 2탭(매니저공지+현장Q&A)만 / 본질 = 네비게이션방 별도 영역 신설
> 3. **허브게시판 부재** — v1 = 개념 자체 부재 / 본질 = 글로벌 데이터 창고 + 보험판 구글 검색창
> 4. **사이트 단위 단순화** — v1 = 전사 글로벌 풀 / 본질 = 지점 단위 격리 + 매니저 공지는 팀 단위 격리
> 5. **dual-write 미러링** — v1 = trigger/dual-write / 본질 = 단일 데이터 + RLS 가시성 분기 (복제 ❌)
> 6. **보험사 임직원-지점 N:M 부재** — v1 = `users.insurer_id` 단일 / 본질 = 다대다 (담당 지점 N개)
> 7. **검색창 사양 부재** — v1 = `keywords TEXT[]` 컬럼만 / 본질 = 현장 Q&A 페이지 큼지막 구글 느낌 검색창
>
> **대체 (현행 진실 원천):** `docs/specs/v2_insurer_admission_phase1_v2.md` (2026-05-08 신설, 42건 결정 통합)
> **보존 사유:** 이력 추적 (즉시 삭제 X). v1 시점 작업 명세는 5/7 commit (`f403b82`) 으로 git 히스토리 보존.
>
> ---

# v2.0 원수사 입점 모델 — Phase 1 통합 spec (v1, 폐기됨)

> **spec명:** `docs/specs/v2_insurer_admission_phase1_v1.md`
> **작성일:** 2026-05-07
> **상태:** ⚠️ **폐기 (2026-05-08, v2 재작성으로 대체)**
> **진실 원천 (현행):** `docs/specs/v2_insurer_admission_phase1_v2.md` ⭐
> **이력 보존:** v1 작성 commit `f403b82` (2026-05-07), patches `9d689f9` `ea8dfb6`
