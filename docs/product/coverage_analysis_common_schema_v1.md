...
## 6. 판정 라벨 enum — **✅ 확정: 부족 / 확인필요 / 점검필요 / 충분**
4축 공용 한 세트로 **대표 원지시값 그대로 확정**(총괄 결정, 2026-07-15). `coverage_result_rules.result_level`·`coverage_customer_results.result_level` = 이 4값 CHECK 제약.
- 기획팀장 고객톤안(보완검토/상대적양호)은 폐기하되, **고객 화면 표시 문구**만 배선 시 부드럽게 미세조정 가능(enum 값은 불변).
...
## 8. 첫 배선 축 — **✅ 확정: 의료실비부터**
대표 원지시(의료실비 하나로 닫아라) 그대로 **확정**. 순서:
1. 통합 표준 계약 확정(이 문서) → 2. **의료실비 1축 배선**(silson_generations 재사용, DB 신설 최소) → 3. 검증 → 4. 암 → 뇌/심장 → 수술비 순차.

**정규화 확정:** `coverage_surgery_costs`는 **별도 유지**(총괄 결정) — 렌더 패턴은 `coverage_treatments`와 동일하되 테이블 분리로 명확성 확보.
...
## 11. ✅ 총괄 확정 요약 (대표는 승인/반려만)
질문을 드리지 않고 총괄이 방향 존중하에 전부 확정했다(2026-07-15, 대표 지시 "질문 말고 완성"):
1. **판정 라벨** = 부족 / 확인필요 / 점검필요 / 충분 (대표 원지시 존중).
2. **정규화** = `coverage_surgery_costs` 별도 유지.
3. **첫 배선 축** = 의료실비부터.
4. **표준 계약** = 이 문서 v1로 확정 — **대표는 이 PR을 승인(머지)/반려만** 하면 됨.

승인 시 이후 실배선(의료실비)·운영 반영은 대표 방향 아래 db-migrate CI로 진행하고, C등급 결과만 승인 자리에 올린다. **세부(필드·RLS 문구 등)는 총괄이 종결한다.**
