...
(function () {
  ...
  function day(value){return new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Seoul',year:'numeric',month:'2-digit',day:'2-digit'}).format(value||new Date());}
  ...
  function stamp(){if(!data)return failed?'갱신 정보를 불러오지 못했습니다. 다시 시도해 주세요.':'기사를 불러오는 중입니다.';var stale=data.date!==day();var warning=collectionStatus&&collectionStatus.date===day()?(collectionStatus.state==='failed'?'자동 수집 실패 · 기존 기사 표시 · ':collectionStatus.state==='degraded'?'일부 수집원 연결 지연 · 대체 수집 반영 · ':''):'';return warning+(stale?'오늘 갱신 대기 · ':'')+new Intl.DateTimeFormat('ko-KR',{timeZone:'Asia/Seoul',month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date(data.updatedAt))+' 갱신';}
  ...
  function load(){if(pending)return pending;if(data&&Date.now()-lastCheck<300000)return Promise.resolve(data);lastCheck=Date.now();pending=Promise.all([read('briefing-latest.json'),read('collection-status.json').catch(function(){return null;})]).then(function(results){collectionStatus=results[1];return results[0];}).then(function(d){if(!Array.isArray(d.items)||!d.updatedAt)throw Error('형식 오류');data=d;failed=false;return d;}).catch(function(){failed=true;return data;}).finally(function(){pending=null;});return pending;}
  ...
  async function mountSection(){var root=document.querySelector('[data-iwb-page]');if(!root)return;var params=new URLSearchParams(location.search);var article=params.get('article')||'';
    ...
    root.innerHTML='<header class="iwb-page-head"><h2>뉴스 브리핑</h2><p class="iwb-note">'+esc(stamp())+'</p><p class="iwb-note">최근 선별된 주요 소식을 최신순으로 쌓아 보여드립니다. 법령·의료 보도는 시행 여부, 적용 대상과 원문을 확인해 주세요.</p></header>'+
      ...
      '<div class="iwb-list">'+(visible.map(function(i){return '<article class="iwb-card"><a href="'+route(i.id,i._briefingDate)+'"><span class="iwb-category">'+esc(i.category)+'</span><h3>'+esc(i.title)+'</h3></a><p class="iwb-note">'+esc(i.sourceType)+' · '+esc(i.source)+' · '+esc(day(new Date(i.publishedAt)))+'</p></article>';}).join('')||'<p class="iwb-note">표시할 기사가 없습니다.</p>')+'</div>'+
  ...
  function card(i,detail){return '<span class="iwb-category">'+esc(i.category)+'</span><h3>'+esc(i.title)+'</h3><p class="iwb-note">'+esc(i.sourceType)+' · '+esc(i.source)+' · '+esc(day(new Date(i.publishedAt)))+'</p>'+(i.description?'<p>'+esc(i.description)+'</p><small>검색 서비스가 제공한 기사 소개문입니다.</small>':'<p>적용 대상·날짜·조건과 자세한 내용은 원문에서 확인해 주세요.</p>')+'<p><a class="iwb-original" href="'+esc(safe(i.url))+'" target="_blank" rel="noopener noreferrer">원문 보기 ↗</a></p>';}
