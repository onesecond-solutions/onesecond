...
export const topics = [
  ...
  { category: '질병·치료·의료비', queries: ['암 신약 건강보험 급여 허가', '뇌졸중 심장질환 치료 건강보험 급여', '질병 치료제 식약처 승인 의료비'], match: /암|백혈병|뇌혈관|뇌졸중|뇌출혈|심장|심혈관|심근|치매|희귀질환|치료|신약|의료비|건강보험.{0,12}급여|급여.{0,12}건강보험/ },
...
const exclude = /\[(사설|칼럼|기고)\]|수 있을까|전문가에게 묻다|전망|가능성|주가|목표주가|영업이익|순이익|배당|주식시장|보험주|증시|시가총액|연예|보험사기단|보험금 노린|살해|이벤트|협찬|특가|프로모션|신간|출간|펴내|출판|특강|금융교육|캠페인|봉사활동|지역 축제|상조회사|선수금|[‘'\"]?[^‘'\"]*보험[’'\"]?\s*(\d+호\s*)?가입|\d+호\s*가입|첫\s*가입|가입\s*행사|출시\s*기념/;
const requiredByCategory = {
  '보험·보상': /실손|보험금|보험료|보장|약관|보험업법|보험 개편|보험가입|보험소비자|금융감독원/,
  '질병·치료·의료비': /암|백혈병|뇌혈관|뇌졸중|뇌출혈|심장|심혈관|심근|치매|희귀질환|질병|환자|치료|신약|치료제|수술|임상|식약처|건강보험.{0,12}급여|급여.{0,12}건강보험|본인부담|의료비/,
...
export function isUsefulTitle(title, category) {
  ...
  if (category === '질병·치료·의료비') {
    if (/보험상품|건강보험상품|보험.{0,8}(출시|판매|가입|고객)|농협.{0,12}보험/.test(text)) return false;
    const diseaseOrTreatment = /암|백혈병|뇌혈관|뇌졸중|뇌출혈|심장질환|심혈관|심근경색|치매|희귀질환|질병|환자|치료제|신약|수술|임상|식약처/;
    const costOrChange = /건강보험.{0,12}급여|급여.{0,12}건강보험|급여화|본인부담|치료비|의료비|승인|허가|등재|적용|지원|확대|축소|개정/;
    if (!diseaseOrTreatment.test(text) || !costOrChange.test(text)) return false;
  }
...
export function normalize(article, topic, now = new Date()) {
  ...
  return { id: createHash('sha256').update(url).digest('hex').slice(0, 20), category: topic.category, title, source, url,
    ...
    priority: 1 + (impact ? 2 : 0) + (medical ? 1 : 0) + (official ? 1 : 0) };
}
