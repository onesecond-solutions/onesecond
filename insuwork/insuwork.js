(function () {
  ...
  function renderAccount() {
    ...
    box.innerHTML = '<button type="button" class="iw-account-trigger" id="iw-account-trigger" aria-haspopup="menu" aria-expanded="false">' + esc(name) + '</button><div class="iw-account-popover" id="iw-account-popover" role="menu" hidden><div class="iw-account-email" aria-label="로그인된 이메일 주소">' + emailLine + '</div><div class="iw-account-group"><div class="iw-account-bgmode"><span>배경화면</span><div class="iw-bgmode-options" role="group" aria-label="배경화면 모드">' + bgModeButtonsHtml(currentBgMode()) + '</div></div></div><div class="iw-account-group"><button type="button" id="iw-profile-open" role="menuitem">개인정보 수정</button></div><div class="iw-account-group"><a class="iw-account-legal" href="/insuwork/business-info.html" target="_blank" rel="noopener" role="menuitem">사업자정보</a><button type="button" id="iw-logout" role="menuitem">로그아웃</button></div></div>';
