...
-- admin 통과 정책 판정 = is_admin() 함수 OR 인라인 EXISTS(users…role='admin') OR true.
--   (라이브 실제 UPDATE 정책은 인라인 EXISTS role='admin' 패턴 — is_admin()만 찾던 이전 PRECHECK 오탐 교정)
do $$
...
begin
  select exists(select 1 from pg_policies
    where schemaname='public' and tablename='newsletters' and cmd in ('SELECT','ALL')
      ...
      and (coalesce(qual,'')='true' or coalesce(qual,'') ilike '%is_admin%'
           or (coalesce(qual,'') ilike '%role%' and coalesce(qual,'') ilike '%admin%'))) into has_sel;
  select exists(select 1 from pg_policies
    where schemaname='public' and tablename='newsletters' and cmd in ('UPDATE','ALL')
      and ('authenticated' = any(roles) or 'public' = any(roles))
      and (coalesce(qual,'') ilike '%is_admin%'
           or (coalesce(qual,'') ilike '%role%' and coalesce(qual,'') ilike '%admin%'))
      and (with_check is null or with_check ilike '%is_admin%'
           or (with_check ilike '%role%' and with_check ilike '%admin%'))) into has_upd;
  ...
  raise notice 'PRECHECK OK: 라이브 newsletters admin SELECT·UPDATE 정책 확인(is_admin 또는 role=admin) → 설치 진행.';
end $$;
