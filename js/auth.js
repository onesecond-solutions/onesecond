  ...
  function appAlert(message, title) {
    ...
    window.setTimeout(function () {
      if (window.InsuranceBriefingNotice && typeof window.InsuranceBriefingNotice.alert === 'function') {
        window.InsuranceBriefingNotice.alert(message, { title: title || '보험워크' });
        return;
      }
      var dialog = document.createElement('dialog');
      dialog.className = 'ib-notice-dialog';
      dialog.innerHTML = '<form class="ib-notice-card" method="dialog"><span class="ib-notice-brand">보험워크</span><h2></h2><p></p><div class="ib-notice-actions"><button type="submit" value="confirm" class="ib-notice-primary">확인</button></div></form>';
      dialog.querySelector('h2').textContent = title || '보험워크';
