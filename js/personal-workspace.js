(function () {
  ...
  function previewDragStart(event) {
    var p = state.preview; if (!p || p.type !== 'pdf' || p.pages <= 1) return;
    state.previewDrag = { x: event.clientX, active: true };
    if (event.currentTarget && event.currentTarget.setPointerCapture) { try { event.currentTarget.setPointerCapture(event.pointerId); } catch (e) {} }
  }
  function previewDragMove(event) {
    var d = state.previewDrag; if (!d || !d.active) return;
    var canvas = document.getElementById('pw-preview-canvas');
    if (canvas) canvas.style.transform = 'translateX(' + Math.max(-120, Math.min(120, event.clientX - d.x)) + 'px)';
  }
  function previewDragEnd(event) {
    var d = state.previewDrag; if (!d || !d.active) return;
    state.previewDrag = null;
    var canvas = document.getElementById('pw-preview-canvas');
    if (canvas) canvas.style.transform = '';
    var delta = event.clientX - d.x;
    if (Math.abs(delta) < 60) return;
    previewPage(delta < 0 ? 1 : -1);
  }
  ...
  function saveCustomerDetail(id) {
    ...
    var name = value('pwd-customer-name'), birth = value('pwd-customer-birth'), date = value('pwd-customer-date'), phone = phoneText(value('pwd-customer-phone')), status = value('pwd-customer-status'), addition = richValue('pwd-customer-new');
    ...
    var note = existingProfile.note || ''; if (richHasText(addition)) note += '<p><strong>[' + esc(writtenAt()) + ']</strong></p>' + addition;
  ...
  function saveConsultationDetail(id) {
    ...
    var name = value('pwd-consult-name'), birth = value('pwd-consult-birth'), date = value('pwd-consult-date'), phone = phoneText(value('pwd-consult-phone')), status = value('pwd-consult-status'), addition = richValue('pwd-consult-new');
    ...
    var content = sanitizeRich(item.memo || ''); if (richHasText(addition)) content += '<p><strong>[' + esc(writtenAt()) + ']</strong></p>' + addition;
