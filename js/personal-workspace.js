(function () {
  ...
  function saveCustomer() { var name = value('pwf-customer-name'), phone = phoneText(value('pwf-customer-phone')), note = richValue('pwf-customer-note'), contractDate = value('pwf-customer-date'), birth = value('pwf-customer-birth'), genderInput = document.querySelector('input[name="pwf-customer-gender"]:checked'), gender = genderInput ? genderInput.value : ''; if (!name || !contractDate) return; write('workspace_customers', { owner_id: currentUserId(), name: name, phone: phone || null, status: value('pwf-customer-status') || '청약완료', profile: { customer_managed: true, contract_date: contractDate, birth_date: birth || null, gender: gender || null, note: sanitizeRich(note) } }).then(function () { resetRichPending(); finishSave('고객을 등록했습니다.'); }).catch(saveError); }
