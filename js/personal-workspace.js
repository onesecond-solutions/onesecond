(function () {
  ...
  var state = {
    section: 'home', assetFilter: 'all', assetView: localStorage.getItem('ws_asset_view') || 'list', assetFolder: null, consultationStatusFilter: 'all', query: '', composing: false, searchTimer: 0,
  ...
  function loadData(force) {
    ...
    state.loadPromise = Promise.allSettled(requests).then(function (results) {
      ...
      var columnSetting = state.data.items.find(function (item) { return item.item_type === 'setting' && item.title === 'consultation_columns'; }); if (columnSetting && columnSetting.body) localStorage.setItem(consultColumnStorageKey(), columnSetting.body);
      ...
      state.data.library = state.data.items.filter(function (item) { return item.item_type !== 'note' && item.item_type !== 'setting'; }).map(function (item) { return Object.assign({}, item, { memo_text: item.item_type === 'memo' ? item.body : null, description: item.body, link_url: item.url, file_url: item.item_type === 'file' ? item.storage_path : null }); });
  ...
  function renderContent() { var main = document.getElementById('pw-main'); if (main) { main.innerHTML = sectionHtml(); if (state.section === 'assets' && state.assetView !== 'list') hydrateAssetThumbs(); if (state.section === 'consultations' && state.selectedConsultation) renderConsultCustomFields(); } }
  function bindSearch() {
  ...
  function formatBirthInput(input, context) { if (!input) return; var raw = String(input.value || '').replace(/\D/g, '').slice(0, 8), formatted = raw; if (raw.length > 4) formatted = raw.slice(0, 4) + '-' + raw.slice(4); if (raw.length > 6) formatted = raw.slice(0, 4) + '-' + raw.slice(4, 6) + '-' + raw.slice(6); input.value = formatted; if (context === 'detail') refreshDetailInsuranceAge(); else refreshInsuranceAge(); }
