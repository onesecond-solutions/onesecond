(function () {
  ...
  function loadData(force) {
    ...
    state.loadPromise = Promise.allSettled(requests).then(function (results) {
      ...
      state.data.library = state.data.items.filter(function (item) { return item.item_type !== 'note' && !isConsultColumnSetting(item) && !isConsultAttachmentItem(item); }).map(function (item) { return Object.assign({}, item, { memo_text: item.item_type === 'memo' ? item.body : null, description: item.body, link_url: item.url, file_url: item.item_type === 'file' ? item.storage_path : null }); });
  ...
  function saveCustomer() { var name = value('pwf-customer-name'), phone = phoneText(value('pwf-customer-phone')), note = richValue('pwf-customer-note'), contractDate = value('pwf-customer-date'), birth = value('pwf-customer-birth'), genderInput = document.querySelector('input[name="pwf-customer-gender"]:checked'), gender = genderInput ? genderInput.value : ''; if (!name || !contractDate) return; write('workspace_customers', { owner_id: currentUserId(), name: name, phone: phone || null, status: value('pwf-customer-status') || '청약완료', profile: { contract_date: contractDate, birth_date: birth || null, gender: gender || null, note: sanitizeRich(note) } }).then(function () { resetRichPending(); finishSave('고객을 등록했습니다.'); }).catch(saveError); }
