(function () {
  ...
  function openVault() {
    ...
    Promise.all([api('myspace_folders?owner_id=eq.' + encodeURIComponent(currentUserId()) + '&deleted_at=is.null&order=name.asc&select=id,name,parent_id,created_at'), api('myspace_files?owner_id=eq.' + encodeURIComponent(currentUserId()) + '&deleted_at=is.null&order=created_at.desc&select=id,original_name,ext,file_size,folder_id,created_at')]).then(function (sets) { state.vaultFolders = sets[0]; state.vaultFiles = sets[1]; renderVault(); }).catch(function () { var content = document.getElementById('pw-vault-content'); if (content) content.innerHTML = '<div class="pw-error">파일함을 불러오지 못했습니다.</div>'; });
  }
  function renderVault() { var content = document.getElementById('pw-vault-content'); if (!content) return; var folders = state.vaultFolders || [], files = state.vaultFiles || []; content.innerHTML = '<div class="pw-vault-grid">' + folders.map(function (folder) { return '<div class="pw-file-card folder"><span>📁</span><b>' + esc(folder.name) + '</b><small>폴더</small></div>'; }).concat(files.map(function (file) { return '<div class="pw-file-card"><span>📄</span><b>' + esc(file.original_name) + '</b><small>' + esc((file.ext || '파일').toUpperCase()) + ' · ' + formatDate(file.created_at) + '</small></div>'; })).join('') + '</div>' + ((!folders.length && !files.length) ? '<div class="pw-empty">저장된 파일이 없습니다.</div>' : ''); }
  function newFolder() { var name = prompt('새 폴더 이름'); if (name == null || !String(name).trim()) return; write('myspace_folders', { owner_id: currentUserId(), parent_id: null, name: String(name).trim(), depth: 0, sort_order: 0 }).then(function () { openVault(); }).catch(saveError); }
  function uploadFiles(files) { var list = Array.prototype.slice.call(files || []); if (!list.length) return; if (typeof window._mvUploadOneFile !== 'function') { alert('파일 업로드 기능을 준비하지 못했습니다.'); return; } var token = window.db.getToken(); Promise.all(list.map(function (file) { return window._mvUploadOneFile(file, null, currentUserId(), token); })).then(function (results) { var failed = results.filter(function (item) { return !item.ok; }); if (failed.length) alert(failed.length + '개 파일을 올리지 못했습니다.'); openVault(); }).catch(saveError); }
  ...
  function saveAsset() { var type = value('pwf-asset-type'), title = value('pwf-title'), body = value('pwf-body'), link = value('pwf-link'); if (!title || !body) return; var request = type === 'note' ? write('scripts', { owner_id: currentUserId(), title: title, script_text: '<p>' + esc(body).replace(/\n/g, '<br>') + '</p>', search_text: title + ' ' + body, scope: 'personal', is_active: true, sort_order: null }) : write('library', { owner_id: currentUserId(), title: title, description: body, memo_text: type === 'memo' ? body : null, link_url: link || null, file_url: null, image_url: null, keywords: [] }); request.then(function () { finishSave('자료를 저장했습니다.'); }).catch(saveError); }
  function saveCustomer() { var name = value('pwf-customer-name'), phone = value('pwf-customer-phone'), note = value('pwf-customer-note'), now = new Date().toISOString(); if (!name) return; write('sales_customers', { owner_id: currentUserId(), name: name, phone: phone || null, phone_raw: phone.replace(/[^0-9]/g, '') || null, status: value('pwf-customer-status') || '신규DB', profile: note ? { note_html: esc(note).replace(/\n/g, '<br>') } : {}, source_ref: null, created_at: now, updated_at: now }).then(function () { finishSave('고객을 등록했습니다.'); }).catch(saveError); }
  function saveConsultation() { var customerId = value('pwf-consult-customer'), memo = value('pwf-consult-memo'), now = new Date().toISOString(); if (!customerId || !memo) return; write('sales_consultations', { customer_id: customerId, owner_id: currentUserId(), consulted_at: now, channel: value('pwf-consult-channel') || null, memo: memo, created_at: now }).then(function () { finishSave('상담 기록을 저장했습니다.'); }).catch(saveError); }
  function saveEvent() { var date = value('pwf-event-date'), title = value('pwf-event-title'); if (!date || !title) return; write('calendar_events', { event_date: date, event_time: value('pwf-event-time') || null, title: title, description: value('pwf-event-desc') || null, event_type: 'operation', source_type: 'personal', audience_target: 'personal', team_id: null, branch_id: null, author_id: currentUserId() }).then(function () { state.selectedDate = date; state.cursor = parseDate(date); finishSave('일정을 추가했습니다.'); }).catch(saveError); }
