(function () {
  ...
  function consultationsHtml() {
    ...
    var list = '<div class="pw-consult-list" role="list">' + rows.map(function (item) {
      ...
      return '<button type="button" role="listitem" class="pw-consult-row' + (String(item.id) === String(state.selectedConsultation) ? ' on' : '') + '" onclick="OSPersonalWorkspace.selectConsultation(\'' + esc(item.id) + '\')"><span class="pw-consult-date">' + esc(date) + '</span><strong>' + esc(customer.name || '(이름 없음)') + '<em>' + esc(profile.gender || '') + '</em></strong><span class="pw-consult-birth">' + esc(profile.birth_date || '') + (age === '' ? '' : ' · 보험나이 ' + age + '세') + '</span><span class="pw-consult-phone">' + esc(phoneText(customer.phone || customer.phone_raw || '')) + '</span><span class="pw-consult-status" data-status="' + esc(status) + '">' + esc(status) + '</span><span class="pw-consult-summary">' + esc(stripHtml(item.memo || '')) + '</span><span class="pw-consult-hover">' + esc(stripHtml(item.memo || '상담내용이 없습니다.')) + '</span></button>';
