(function () {
  ...
  function customersHtml() {
    ...
    var stats = statFilterBarHtml({ kind: 'customer', stages: CUSTOMER_STAGES, activeStatus: state.customerStatusFilter, counts: counts, nameQuery: state.customerNameQuery, nameInputId: 'iw-customer-name-input', namePlaceholder: '고객명·전화번호 검색', onStage: 'OSInsuwork.filterCustomerStatus' });
    return '<div class="iw-consult-screen">' + statusHtml() + '<div class="iw-toolbar"><h2>고객관리</h2><div class="iw-toolbar-actions"><button class="iw-btn primary" onclick="OSInsuwork.addCustomer()">+ 고객 등록</button><button type="button" class="iw-btn iw-fav-toggle" onclick="OSInsuwork.toggleFavoritesPanel(event)" aria-haspopup="true" aria-expanded="false" title="즐겨찾기">★</button><div class="iw-fav-panel" id="iw-fav-panel" hidden></div></div></div>' + customersCareNoticeHtml() + stats + '<div class="iw-consult-layout' + (selected ? ' has-detail' : '') + '"><section class="iw-consult-master"><div class="iw-consult-list" role="list">' + header + '<div class="iw-consult-rows">' + body + (rows.length ? '' : '<div class="iw-empty">등록된 고객이 없습니다.</div>') + '</div>' + loadMoreHtml(totalRowCount, rows.length, 'OSInsuwork.loadMoreCustomers()') + '</div></section>' + detail + '</div></div>';
  }
  ...
  function consultationsHtml() {
    ...
    var stats = statFilterBarHtml({ kind: 'consult', stages: CONSULT_STAGES, activeStatus: state.consultationStatusFilter, counts: counts, nameQuery: state.consultNameQuery, nameInputId: 'iw-consult-name-input', namePlaceholder: '고객명·전화번호 검색', onStage: 'OSInsuwork.filterConsultationStatus' });
    return '<div class="iw-consult-screen">' + statusHtml() + '<div class="iw-toolbar"><h2>상담관리</h2><div class="iw-toolbar-actions"><button class="iw-btn primary" onclick="OSInsuwork.addConsultation()">+ 상담 등록</button><button type="button" class="iw-btn iw-fav-toggle" onclick="OSInsuwork.toggleFavoritesPanel(event)" aria-haspopup="true" aria-expanded="false" title="즐겨찾기">★</button><div class="iw-fav-panel" id="iw-fav-panel" hidden></div></div></div>' + stats + '<div class="iw-consult-layout' + (selected ? ' has-detail' : '') + '"><section class="iw-consult-master">' + list + '</section>' + detail + '</div></div>';
  }
