(function () {
  ...
  function loadData(force) {
    ...
    var requests = full ? [
      ...
      api('insuwork_customers?owner_id=eq.' + id + '&deleted_at=is.null&order=updated_at.desc&limit=2000&select=id,owner_id,name,phone,status,profile,created_at,updated_at,deleted_at')
    ];
  ...
  function customersHtml() {
    ...
    var baseRows = state.data.customers.filter(function (item) { if (nameQ && searchNorm((item.name || '') + ' ' + (item.phone || item.phone_raw || '')).indexOf(nameQ) < 0) return false; return true; });
