(function () {
  ...
  function searchHtml() {
    ...
    if (state.briefingSearchQuery === searchNorm(q)) state.briefingSearchRows.forEach(function (item) {
      results.push({ icon: '◫', kind: '보험이슈' + (item.category ? ' · ' + item.category : ''), title: item.title, sub: [item.insurer, item.received_date, item.snippet].filter(Boolean).join(' · '), action: "OSInsuwork.openBriefingSearchResult('" + esc(item.id) + "')" });
    });
    var waiting = state.briefingSearchLoading ? '<div class="iw-sync-note">보험이슈 자료를 검색하고 있습니다.</div>' : '';
    return '<div class="iw-toolbar"><div><h2>‘' + esc(q) + '’ 검색 결과</h2><p class="iw-subtitle">자료, 계약, 상담, 일정, 보험이슈, 원전산과 결제정보를 한 번에 검색했습니다.</p></div><span class="iw-result-count">' + results.length + '건</span></div>' + waiting + '<div class="iw-search-results">' + (results.length ? results.map(function (item) { return '<button type="button" onclick="' + item.action + '"><span class="iw-result-icon">' + item.icon + '</span><span><small>' + item.kind + '</small><b>' + esc(item.title || '(제목 없음)') + '</b><em>' + esc(item.sub) + '</em></span><span>›</span></button>'; }).join('') : (state.briefingSearchLoading ? '' : '<div class="iw-empty"><strong>검색 결과가 없습니다.</strong><span>띄어쓰기나 검색어를 바꿔 보세요.</span></div>')) + '</div>';
  }
