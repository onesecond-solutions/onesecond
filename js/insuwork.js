(function () {
  ...
  function familySectionHtml(customer) {
    ...
    return '<section class="iw-family-section"><button type="button" class="iw-family-section-toggle" aria-expanded="false" aria-controls="' + esc(panelId) + '" onclick="OSInsuwork.toggleFamilySection(this,\'' + esc(panelId) + '\')"><span><strong>가족 추가</strong>' + summary + '</span><b aria-hidden="true">+</b></button>'
      + '<div class="iw-family-body" id="' + esc(panelId) + '" hidden><div class="iw-family-members">' + memberRows + '</div>' + candidateHtml + '<div class="iw-family-connect"><select id="iwd-family-customer" aria-label="연결할 기존 고객"><option value="">기존 고객 선택</option>' + options + '</select><select id="iwd-family-relation" aria-label="가족 관계"><option>배우자</option><option>부모</option><option>자녀</option><option>형제자매</option><option>기타</option></select><button type="button" class="iw-btn" onclick="OSInsuwork.connectFamily(\'' + esc(customer.id) + '\')">가족 연결</button></div></div></section>';
  }
  function toggleFamilySection(button, panelId) {
    ...
    if (opening) { var select = panel.querySelector('#iwd-family-customer'); if (select) select.focus({ preventScroll: true }); }
  }
  ...
  function connectFamily(customerId) {
    ...
    saveFamilyProfiles(updates).then(function () { renderContent(); if (typeof window.toast === 'function') window.toast(customer.name + ' 고객과 ' + target.name + ' 고객을 가족으로 연결했습니다.'); }).catch(saveError);
  }
