(function () {
  ...
  function searchResultCardHtml(item) {
    ...
    return '<button type="button" class="iw-asset-card iw-search-result-card" onclick="' + item.action + '"><span class="iw-asset-preview">' + preview + '</span><b>' + esc(item.title || '(제목 없음)') + '</b><small>' + esc(item.kind) + '</small></button>';
  }
