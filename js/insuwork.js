(function () {
  ...
  function kakaoHubHtml() {
    ...
    return '<div class="iw-kakao-hub"><button type="button" class="iw-kakao-hub-toggle' + tone + '" onclick="OSInsuwork.toggleKakaoHub(event)" aria-haspopup="true" aria-expanded="false" title="카카오 업무함"><span>톡</span>' + (actionCount ? '<b>' + actionCount + '</b>' : '') + '</button><section class="iw-kakao-hub-panel" id="iw-kakao-hub-panel" hidden><header><strong>카카오 업무함</strong><button type="button" onclick="OSInsuwork.toggleKakaoHub(event,false)" aria-label="닫기">×</button></header><div class="iw-kakao-hub-stats"><span><b>' + unread + '</b>새 문의</span><span><b>' + pending + '</b>발송 대기</span><span><b>' + failed + '</b>확인 필요</span></div><div class="iw-kakao-hub-tabs"><button type="button" class="on">전체</button><button type="button">알림톡</button><button type="button">친구톡</button><button type="button">상담톡</button></div><div class="iw-kakao-hub-list">' + list + '</div></section></div>';
  }
