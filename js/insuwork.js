(function () {
  ...
  function familySectionHtml(customer) {
    ...
    var candidateHtml = candidates.length ? '<div class="iw-family-candidates"><strong>가족 연결 후보</strong><p>고객내용에서 가족 표현과 이름이 함께 발견됐습니다. 확인 후 연결하세요.</p>' + candidates.map(function (entry) { return '<button type="button" onclick="OSInsuwork.prepareFamilyCandidate(\'' + esc(entry.id) + '\')">' + esc(entry.name || '이름 없음') + ' 연결 검토</button>'; }).join('') + '</div>' : '';
