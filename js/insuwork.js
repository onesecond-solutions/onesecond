(function () {
  ...
  var state = {
    ...
    adminUsers: null, adminUsersLoading: false, adminUsersError: '', adminUserQuery: '', adminUserStatus: 'all', adminUserComposing: false, adminUserTimer: 0, adminAzRoomMembers: {}, azRoomPermissionSaving: {}, azViewingRoomAccess: null, azRoomTool: '',
  ...
  function azViewingRoomHtml() {
    var content = state.azRoomTool === 'capture'
      ? '<nav class="iw-az-room-breadcrumb" aria-label="현재 위치"><button type="button" onclick="OSInsuwork.openAzRoomTool(\'\')">에즈 시청방</button><span aria-hidden="true">|</span><strong>스크롤 캡처</strong></nav>' + toolsCaptureHtml()
      : '<div class="iw-az-room-tools" aria-label="에즈 시청방 기능 목록"><button type="button" class="iw-az-room-tool-card" onclick="OSInsuwork.openAzRoomTool(\'capture\')"><strong>스크롤 캡처</strong><span>해피톡·보맵플래너 채팅 화면 저장</span></button></div>';
    return '<div class="iw-az-room-page"><div class="iw-toolbar"><div><h2>에즈 시청방</h2><p class="iw-subtitle">허용된 멤버만 이용할 수 있는 팀 전용 공간입니다.</p></div></div><section class="iw-az-room-workspace" aria-label="에즈 시청방 작업 공간">' + content + '</section></div>';
  }
  function openAzRoomTool(tool) {
    if (!canSeeAzViewingRoom()) return;
    state.azRoomTool = tool === 'capture' ? 'capture' : '';
    renderContent();
  }
