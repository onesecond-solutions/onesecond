(function () {
  ...
  function applicationCalendarEventsForYear(year) {
    var out = [];
    state.data.customers.forEach(function (customer) {
      contractDatesOf(customer).forEach(function (date) {
  ...
  function showSearchImageHover(event) {
    ...
    var rect = card.getBoundingClientRect(), width = Math.min(420, Math.max(260, window.innerWidth * 0.3)), gap = 14;
    ...
    tip.style.width = width + 'px'; tip.style.left = left + 'px'; tip.style.top = Math.max(gap, Math.min(rect.top, window.innerHeight - Math.min(560, window.innerHeight - gap * 2) - gap)) + 'px';
  }
