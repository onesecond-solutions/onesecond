(function () {
  ...
  var CONSULT_BASE_COLUMNS = [{ key: 'date', label: '등록일자', width: 86 }, { key: 'name', label: '이름', width: 200 }, { key: 'birth', label: '생년월일', width: 92 }, { key: 'genderAge', label: '성별(보험나이)', width: 104 }, { key: 'phone', label: '전화번호', width: 116 }, { key: 'summary', label: '상담내용', width: 308, flex: true }, { key: 'status', label: '상담상태', width: 102 }];
  ...
  function canSeeAzViewingRoom() { return false; }
  ...
  function isBusinessCardSetting(item) { var payload = item && item.legacy_payload || {}; return payload.workspace_category === 'settings' && payload.setting_key === 'business_card'; }
  function isWorkspaceSetting(item) { var payload = item && item.legacy_payload || {}; return item && payload.workspace_category === 'settings'; }
  ...
  function isKakaoPilotItem(item) { var payload = item && item.legacy_payload || {}; return payload.workspace_category === 'kakao_message_pilot'; }
  ...
  function canEnterSection(section) { if (section === 'insurance-portal') return canEditCoverageTemplate(); if (section === 'ledger') return !!(window._canSeeInsuworkLedger && window._canSeeInsuworkLedger()); if (section === 'coverage-analysis') return canUseCoverageAnalysis(); if (section === 'coverage-sheet') return canEditCoverageTemplate(); if (section === 'az-viewing-room') return canSeeAzViewingRoom(); if (section === 'admin-users' || section === 'personal-office' || section === 'my-work') return localPreviewAllowed() || (authenticated() && canSeeAdminUsers()); return PROTECTED_SECTIONS.indexOf(section) < 0 || allowed(); }
  ...
  function familyGroupId(customer) { return String(customerProfile(customer).family_group_id || ''); }
  function familyMembers(customer) {
  ...
  function familyRelation(customer, memberId) {
    ...
    return found && found.relation || '가족';
  }
  function reciprocalFamilyRelation(relation) {
    return { 배우자: '배우자', 부모: '자녀', 자녀: '부모', 형제자매: '형제자매', 기타: '기타' }[relation] || '가족';
  }
  function familyBadgeHtml(customer) {
    var count = familyMembers(customer).length;
    return count > 1 ? '<span class="iw-family-badge" role="button" tabindex="0" onclick="event.stopPropagation();OSInsuwork.showFamilyGroup(\'' + esc(customer.id) + '\')" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();event.stopPropagation();OSInsuwork.showFamilyGroup(\'' + esc(customer.id) + '\')}">가족 ' + count + '명</span>' : '';
  }
  ...
  function needsFullItems() {
    return !!state.query.trim() || ['insurance-portal', 'assets', 'coverage-analysis', 'customers', 'consultations', 'trash'].indexOf(state.section) >= 0 || /\/m\/(library|search|customers|consultations)\.html$/.test(location.pathname);
  }
  ...
  function navHtml() {
    ...
    if (canEditCoverageTemplate()) items.push(['insurance-portal', '◈', '보험포털']);
  ...
  var HELP_CONTENT = {
    ...
    customers: { title: '케어일정 자동 생성', preview: '청약완료로 등록하면 케어 일정이 자동으로 만들어져요.', body: '청약완료로 등록하면 청약일 기준 <strong>31·91·181·365일 케어 일정</strong>이 캘린더에 만들어집니다. 청약일은 최초 날짜에만 표시하며 매년 반복하지 않습니다.<br>별도 설정은 필요 없습니다.' },
  ...
  function searchHtml() {
    ...
    var results = [];
    if (canEnterSection('insurance-portal') && window.OSInsurancePortal) window.OSInsurancePortal.entries().forEach(function (a) { if (matches(a.title + ' ' + a.description)) results.push({icon:'◈',kind:'보험포털',title:a.title,sub:'주제별 설명과 도표',raw:{},action:"OSInsurancePortal.open('" + a.id + "')"}); });
    loadPaymentInfo();
    state.data.scripts.forEach(function (item) { if (matches((item.title || '') + ' ' + stripHtml(item.script_text))) results.push({ icon: '📝', kind: '업무노트', title: item.title, sub: formatDate(item.created_at), action: "OSInsuwork.showAsset('scripts','" + esc(item.id) + "')", raw: item }); });
    state.data.library.forEach(function (item) { if (matches((item.title || '') + ' ' + (item.description || '') + ' ' + (item.memo_text || ''))) results.push({ icon: '📄', kind: item.memo_text ? '메모' : '자료', title: item.title, sub: formatDate(item.created_at), action: "OSInsuwork.showAsset('library','" + esc(item.id) + "')", raw: item }); });
    ...
    var switcher = '<div class="iw-view-switch" aria-label="검색결과 보기 방식">' + modes.map(function (mode) { return '<button type="button" class="' + (state.searchView === mode[0] ? 'on' : '') + '" onclick="OSInsuwork.setSearchView(\'' + mode[0] + '\')" aria-label="' + mode[1] + ' 보기" title="' + mode[1] + '"><span aria-hidden="true">' + mode[2] + '</span>' + mode[1] + '</button>'; }).join('') + '</div>';
    var empty = state.briefingSearchLoading ? '' : '<div class="iw-empty"><strong>검색 결과가 없습니다.</strong><span>띄어쓰기나 검색어를 바꿔 보세요.</span></div>';
    var content = state.searchView === 'list' ? '<div class="iw-search-results">' + (results.length ? results.map(searchResultRowHtml).join('') : empty) + '</div>' : '<div class="iw-assets-grid iw-search-result-grid ' + (state.searchView === 'large' ? 'large' : '') + '">' + (results.length ? results.map(searchResultCardHtml).join('') : empty) + '</div>';
    return '<div class="iw-toolbar iw-search-toolbar"><div><h2>‘' + esc(q) + '’ 검색 결과</h2><p class="iw-subtitle">자료, 계약, 상담, 일정, 보험이슈, 원전산과 결제정보를 한 번에 검색했습니다.</p></div><div class="iw-search-view-actions"><span class="iw-result-count">' + results.length + '건</span>' + switcher + '</div></div>' + waiting + content;
  }

  function searchResultRowHtml(item) {
    return '<button type="button" onclick="' + item.action + '"><span class="iw-result-icon">' + item.icon + '</span><span><small>' + esc(item.kind) + '</small><b>' + esc(item.title || '(제목 없음)') + '</b><em>' + esc(item.sub) + '</em></span><span>›</span></button>';
  }

  function searchResultCardHtml(item) {
    var raw = item.raw || {}, direct = raw.image_url || (/\.(png|jpe?g|gif|webp)(\?.*)?$/i.test(raw.url || '') ? raw.url : '');
    var briefingImage = raw.storage_path && /^image\//.test(raw.mime_type || '') && item.kind.indexOf('보험이슈') === 0;
    if (!direct && briefingImage && window.SUPABASE_URL) direct = window.SUPABASE_URL + '/storage/v1/object/public/briefing-leaflets/' + String(raw.storage_path).split('/').map(encodeURIComponent).join('/');
    var storedImage = raw.storage_path && /^image\//.test(raw.mime_type || '') && !briefingImage;
    var image = direct ? '<img src="' + esc(direct) + '" alt="">' : storedImage ? '<img data-storage-path="' + esc(raw.storage_path) + '" alt="">' : '';
    var preview = image || '<div class="iw-asset-document iw-search-document"><span class="iw-search-document-icon">' + item.icon + '</span><strong>' + esc(item.kind) + '</strong><p>' + esc(String(item.sub || '').slice(0, 150)) + '</p></div>';
    var hover = image ? ' onmouseenter="OSInsuwork.showSearchImageHover(event)" onmouseleave="OSInsuwork.hideSearchImageHover()"' : '';
    return '<button type="button" class="iw-asset-card iw-search-result-card" onclick="' + item.action + '"' + hover + '><span class="iw-asset-preview">' + preview + '</span><b>' + esc(item.title || '(제목 없음)') + '</b><small>' + esc(item.kind) + '</small></button>';
  }
  ...
  function homeTodayColumnIndex(event) {
    ...
    return kind === 'birthday' ? 3 : kind === 'age' ? 2 : kind === 'care' ? 1 : 0;
  }
  function homeTodayChip(event) {
    var kind = homeTodayKind(event);
    var cls = kind === 'birthday' ? 'birthday' : kind === 'age' ? 'insurance-age' : kind === 'care' ? 'customer' : 'schedule';
  ...
  function resolveFavorite(type, id) {
    if (type === 'asset') {
      ...
      if (asset) return { action: function () { if (asset.item_type === 'folder') { state.section = 'assets'; state.assetFolder = id; state.assetFilter = assetCategory(asset); renderContent(); setUrl(false); } else showAsset('library', id); } };
    }
  ...
  function birthdayCalendarEventsForYear(year) {
    var out = [];
    state.data.customers.forEach(function (customer) {
      var profile = customerProfile(customer), birth = String(profile.birth_date || '').slice(0, 10), parts = birth.split('-').map(Number), born = parseDate(birth);
      if (!/^\d{4}-\d{2}-\d{2}$/.test(birth) || parts.length !== 3 || born.getFullYear() !== parts[0] || born.getMonth() !== parts[1] - 1 || born.getDate() !== parts[2]) return;
      var birthday = new Date(year, parts[1] - 1, parts[2]);
      if (birthday.getMonth() !== parts[1] - 1) birthday = new Date(year, parts[1], 0);
      var name = customer.name || '고객', phone = phoneText(customer.phone || customer.phone_raw || ''), date = ymd(birthday);
      out.push({ id: 'birthday-' + customer.id + '-' + year, customer_id: customer.id, event_date: date, event_end_date: date, title: name + ' 생일', event_type: 'birthday', description: name + ' 고객 생일입니다. 생년월일 ' + birth + (phone ? ' · 연락처: ' + phone : ''), builtin: true });
    });
    return out;
  }
  function applicationCalendarEventsForYear(year) {
    var out = [];
    state.data.customers.forEach(function (customer) {
      contractDatesOf(customer).slice(0, 1).forEach(function (date) {
        ...
        out.push({ id: 'application-' + customer.id + '-' + date, customer_id: customer.id, event_date: date, event_end_date: date, title: name + ' 청약', event_type: 'application', description: name + ' 고객 청약일자입니다.' + (phone ? ' 연락처: ' + phone : ''), builtin: true });
      });
    });
    return out;
  }
  ...
  function builtInEventsAroundCalendar() {
    ...
    return Object.keys(years).reduce(function (rows, year) { year = Number(year); return rows.concat(builtinCalendarEvents(year), insuranceAgeCalendarEventsForYear(year), birthdayCalendarEventsForYear(year), applicationCalendarEventsForYear(year)); }, []);
  }
  function obsoleteApplicationAnniversary(event) { return event.legacy_source === 'care_auto' && String(event.legacy_id || '').indexOf(':anniversary:') >= 0; }
  function allEvents() { return state.data.events.filter(function (event) { return !obsoleteApplicationAnniversary(event); }).concat(builtInEventsAroundCalendar()); }
  ...
  function assetsHtml() {
    if (canUseAssetActions() && !state.assetCardsInitialized) { state.assetView = localStorage.getItem('iw_asset_card_view') || 'thumb'; state.assetCardsInitialized = true; }
    var items = [];
    ...
    var viewModes = [['list', '목록', '☷'], ['thumb', canUseAssetActions() ? '카드' : '썸네일', '▦'], ['large', '큰 이미지', '▣']];
    ...
    var content = state.assetView === 'list'
      ? '<div class="iw-explorer"><table class="iw-table"><thead><tr><th>이름</th><th>종류</th><th>현재 분류</th><th>등록일</th>' + (canUseAssetActions() ? '<th aria-label="자료 메뉴"></th>' : '') + '</tr></thead><tbody>' + items.map(function (item) { return '<tr tabindex="0" class="' + (item.folder ? 'iw-folder-drop-target' : 'iw-asset-draggable') + (selectedNote && selectedNote.source === item.source && String(selectedNote.id) === String(item.raw.id) ? ' iw-note-selected' : '') + '" data-note-id="' + esc(item.raw.id) + '" data-note-source="' + item.source + '" ' + assetDragAttributes(item) + ' onclick="' + assetOpenAction(item) + '"><td><span class="iw-title-with-fav">' + (item.folder ? '' : favoriteButton('asset', item.raw.id, item.title || '(제목 없음)', item.kind + ' · ' + formatDate(item.created))) + '<b>' + (item.folder ? '📁 ' : '') + esc(item.title || '(제목 없음)') + '</b></span></td><td>' + item.kind + '</td><td>' + scopeBadge(item.raw) + '</td><td>' + formatDate(item.created) + '</td>' + (canUseAssetActions() ? '<td>' + assetMoreButton(item) + '</td>' : '') + '</tr>'; }).join('') + '</tbody></table>' + (items.length ? '' : '<div class="iw-empty">조건에 맞는 자료가 없습니다.</div>') + '</div>'
      : (canUseAssetActions() ? assetCardsGrid(items) : '<div class="iw-assets-grid ' + (state.assetView === 'large' ? 'large' : '') + '">' + items.map(assetCardHtml).join('') + (items.length ? '' : '<div class="iw-empty">조건에 맞는 자료가 없습니다.</div>') + '</div>');
  ...
  function assetCardHtml(item) {
    ...
    if (canUseAssetActions() && item.body && !image) docBody = '<p>' + esc(stripHtml(String(item.body)).slice(0,180)) + '</p>';
  ...
  function assetMoreButton(item) {
    return '<button type="button" class="iw-asset-more" aria-label="' + esc(item.title || '자료') + ' 더보기" aria-haspopup="menu" aria-expanded="false" data-asset-id="' + esc(item.raw.id) + '" onclick="event.stopPropagation();OSInsuwork.assetMore(event,this.dataset.assetId)" onpointerdown="event.stopPropagation()" onkeydown="event.stopPropagation()">⋮</button>';
  }
  function assetActionCard(item, preview) {
    var action = assetOpenAction(item), title = esc(item.title || '(제목 없음)');
    return '<article class="iw-asset-action-card' + (item.folder ? ' is-folder iw-folder-drop-target' : ' iw-asset-draggable') + '" ' + assetDragAttributes(item) + '><div class="iw-asset-card-heading"><button type="button" class="iw-asset-title-open" title="' + title + '" onclick="' + action + '">' + title + '</button>' + assetMoreButton(item) + '</div>' + (item.folder ? '' : '<button type="button" class="iw-asset-content-open" aria-label="' + title + ' 열기" onclick="' + action + '"><span class="iw-asset-preview">' + preview + '</span></button><small>' + esc(item.kind) + ' · ' + formatDate(item.created) + '</small>') + '</article>';
  }
  function assetCardsGrid(items) {
    if (!items.length) return '<div class="iw-empty">조건에 맞는 자료가 없습니다.</div>';
    var folders = items.filter(function (i) { return i.folder; }), files = items.filter(function (i) { return !i.folder; });
    return (folders.length ? '<div class="iw-asset-card-grid iw-asset-folder-grid" aria-label="폴더">' + folders.map(assetCardHtml).join('') + '</div>' : '') + (files.length ? '<div class="iw-asset-card-grid' + (state.assetView === 'large' ? ' is-large' : '') + '" aria-label="자료 카드">' + files.map(assetCardHtml).join('') + '</div>' : '');
  }
  var assetMenuTarget = null, assetMenuTrigger = null, assetMoveState = null;
  function closeAssetActions() {
    var menu = document.getElementById('iw-asset-action-menu');
    if (menu) { if (menu.hidePopover && menu.matches(':popover-open')) menu.hidePopover(); menu.remove(); }
    if (assetMenuTrigger && assetMenuTrigger.isConnected) assetMenuTrigger.setAttribute('aria-expanded','false');
  }
  function assetMore(event, id) {
    if (event) event.stopPropagation();
    var item = actionableAsset(id); if (!item) return;
    var existingMenu = document.getElementById('iw-asset-action-menu');
    var trigger = event.currentTarget, wasOpen = assetMenuTarget === id && existingMenu && (!existingMenu.showPopover || existingMenu.matches(':popover-open'));
    closeAssetActions(); if (wasOpen) { assetMenuTarget = null; return; }
    assetMenuTarget = id; assetMenuTrigger = trigger;
    var entries = [['rename','이름 바꾸기'],['move','폴더로 이동'],['favorite',isFavorited('asset',id) ? '즐겨찾기 해제' : '즐겨찾기 추가']];
    if (['memo','note','link'].indexOf(item.item_type) >= 0) entries.unshift(['edit',item.item_type === 'link' ? '링크 수정' : '내용 수정']);
    if (item.item_type === 'file' && item.storage_path) entries.push(['download','다운로드']);
    if (item.url && /^https?:\/\//i.test(item.url)) entries.push(['copy','주소 복사']);
    entries.push(['info','정보 보기'],['trash','휴지통으로 이동']);
    var menu = document.createElement('div'); menu.id = 'iw-asset-action-menu'; menu.className = 'iw-asset-action-menu'; menu.setAttribute('role','menu'); menu.setAttribute('aria-label',item.title + ' 메뉴');
    menu.innerHTML = entries.map(function (entry) { return '<button type="button" role="menuitem" class="' + (entry[0] === 'trash' ? 'is-danger' : '') + '" data-action="' + entry[0] + '" onclick="OSInsuwork.assetMenuAction(this.dataset.action)">' + entry[1] + '</button>'; }).join('');
    document.getElementById('v-insuwork').appendChild(menu);
    if (menu.showPopover) { menu.setAttribute('popover','auto'); menu.showPopover(); }
    var rect = trigger.getBoundingClientRect(); menu.style.left = Math.max(8,Math.min(rect.right-menu.offsetWidth,window.innerWidth-menu.offsetWidth-8))+'px'; menu.style.top = Math.max(8,Math.min(rect.bottom+4,window.innerHeight-menu.offsetHeight-8))+'px';
    trigger.setAttribute('aria-expanded','true');
    menu.addEventListener('toggle',function (e) { if (e.newState === 'closed' && trigger.isConnected) trigger.setAttribute('aria-expanded','false'); });
    menu.addEventListener('keydown',function (e) {
      var buttons = Array.from(menu.querySelectorAll('button')), index = buttons.indexOf(document.activeElement);
      if (e.key === 'Escape' || e.key === 'Tab') { closeAssetActions(); if (e.key === 'Escape') { e.preventDefault(); trigger.focus(); } return; }
      if (['ArrowDown','ArrowUp','Home','End'].indexOf(e.key) < 0) return;
      e.preventDefault(); buttons[e.key === 'Home' ? 0 : e.key === 'End' ? buttons.length-1 : (index+(e.key === 'ArrowDown'?1:-1)+buttons.length)%buttons.length].focus();
    });
    menu.querySelector('button').focus();
  }
  function assetActionRefresh(message) {
    var main = document.querySelector('#v-insuwork .iw-main'), top = main ? main.scrollTop : 0;
    renderContent(); requestAnimationFrame(function () { var next = document.querySelector('#v-insuwork .iw-main'); if (next) next.scrollTop = top; });
    if (typeof window.toast === 'function') window.toast(message);
  }
  function assetMenuAction(action) {
    var item = actionableAsset(assetMenuTarget); closeAssetActions(); if (!item) return;
    if (action === 'edit') return editAsset(item.id);
    if (action === 'move') return assetMoveOpen(item.id);
    if (action === 'favorite') return toggleFavorite('asset',item.id,item.title,assetCategoryLabel(assetCategory(item)));
    if (action === 'trash') return item.item_type === 'folder' ? deleteAssetFolder(item.id) : deleteAsset(item.id);
    if (action === 'rename') return briefingPrompt('새 이름을 입력하세요.','이름 바꾸기',item.title || '').then(function (name) {
      if (name == null || !String(name).trim() || String(name).trim() === item.title) return;
      if (!actionableAsset(item.id)) throw new Error('수정할 자료를 확인하지 못했습니다.');
      return updateOne('insuwork_items?id=eq.'+encodeURIComponent(item.id)+'&owner_id=eq.'+encodeURIComponent(currentUserId())+'&deleted_at=is.null',{title:String(name).trim()}).then(function (updated) { upsertWorkspaceItem(updated); syncPublicLibraryItem(updated); assetActionRefresh('이름을 변경했습니다.'); });
    }).catch(saveError);
    if (action === 'copy') return navigator.clipboard.writeText(item.url).then(function () { if (typeof window.toast === 'function') window.toast('주소를 복사했습니다.'); }).catch(saveError);
    if (action === 'download') return signStoragePath(item.storage_path).then(function (url) { return fetch(url); }).then(function (r) { if (!r.ok) throw new Error('파일을 내려받지 못했습니다.'); return r.blob(); }).then(function (blob) { var url = URL.createObjectURL(blob); downloadBlob(url,item.title || '파일'); window.setTimeout(function () { URL.revokeObjectURL(url); },60000); }).catch(saveError);
    if (action === 'info') return dialog('<div class="iw-form"><h2>자료 정보</h2><p>' + esc(item.title) + '</p><p>종류: ' + esc(item.item_type === 'folder' ? '폴더' : fileExtension(item) || assetCategoryLabel(assetCategory(item))) + '</p><p>위치: ' + esc(assetLocation(item.parent_id,assetCategory(item))) + '</p><p>등록일: ' + esc(formatDate(item.created_at)) + '</p>' + (item.file_size != null ? '<p>크기: ' + esc(formatBytes(item.file_size)) + '</p>' : '') + '<button type="button" class="iw-btn" onclick="OSInsuwork.closeDialog()">닫기</button></div>');
  }
  function assetLocation(folderId, category) {
    var names = [], seen = new Set();
    while (folderId && !seen.has(String(folderId))) { seen.add(String(folderId)); var folder = workspaceItem(folderId); if (!folder) break; names.unshift(folder.title); folderId = folder.parent_id; }
    return [assetCategoryLabel(category)].concat(names).join(' / ');
  }
  function validAssetDestination(item, folderId) {
    if (!folderId) return true;
    var folder = actionableAsset(folderId); if (!folder || folder.item_type !== 'folder' || assetCategory(folder) !== assetCategory(item)) return false;
    var seen = new Set(), current = folder;
    while (current) { if (String(current.id) === String(item.id) || seen.has(String(current.id))) return false; seen.add(String(current.id)); current = current.parent_id ? workspaceItem(current.parent_id) : null; }
    return true;
  }
  function assetMoveOpen(id) {
    var item = actionableAsset(id); if (!item) return;
    assetMoveState = {id:id,folder:item.parent_id || null,busy:false}; assetMoveRender();
  }
  function assetMoveRender() {
    var move = assetMoveState, item = move && actionableAsset(move.id); if (!item) return;
    var category = assetCategory(item), parent = move.folder && workspaceItem(move.folder), folders = state.data.items.filter(function (folder) { return folder.item_type === 'folder' && String(folder.parent_id || '') === String(move.folder || '') && validAssetDestination(item,folder.id); });
    dialog('<div class="iw-form iw-asset-move"><h2>폴더로 이동</h2><p>' + esc(item.title) + '</p><p aria-live="polite">' + esc(assetLocation(move.folder,category)) + '</p><div class="iw-actions">' + (move.folder ? '<button type="button" class="iw-btn" data-parent="' + esc(parent && parent.parent_id || '') + '" onclick="OSInsuwork.assetMoveEnter(this.dataset.parent)">상위 폴더</button>' : '') + '<button type="button" class="iw-btn" onclick="OSInsuwork.assetMoveNewFolder()">새 폴더</button></div><div class="iw-asset-move-folders">' + folders.map(function (folder) { return '<button type="button" class="iw-btn" data-folder="' + esc(folder.id) + '" onclick="OSInsuwork.assetMoveEnter(this.dataset.folder)">' + esc(folder.title) + ' ›</button>'; }).join('') + (folders.length ? '' : '<p>이 위치에 하위 폴더가 없습니다.</p>') + '</div><div class="iw-actions"><button type="button" class="iw-btn" onclick="OSInsuwork.closeDialog()">취소</button><button type="button" class="iw-btn primary"' + (String(item.parent_id || '') === String(move.folder || '') ? ' disabled' : '') + ' onclick="OSInsuwork.assetMoveConfirm()">여기로 이동</button></div></div>');
  }
  function assetMoveEnter(id) { var m = assetMoveState, item = m && actionableAsset(m.id); if (!item || m.busy || !validAssetDestination(item,id)) return; m.folder = id || null; assetMoveRender(); }
  function assetMoveConfirm() {
    var move = assetMoveState, item = move && actionableAsset(move.id); if (!item || move.busy || !validAssetDestination(item,move.folder) || String(item.parent_id || '') === String(move.folder || '')) return;
    move.busy = true;
    return updateOne('insuwork_items?id=eq.'+encodeURIComponent(item.id)+'&owner_id=eq.'+encodeURIComponent(currentUserId())+'&deleted_at=is.null',{parent_id:move.folder || null}).then(function (updated) { upsertWorkspaceItem(updated); closeDialog(); assetActionRefresh('이동 완료 · '+assetLocation(move.folder,assetCategory(item))); }).catch(saveError).finally(function () { move.busy = false; });
  }
  function assetMoveNewFolder() {
    var move = assetMoveState, item = move && actionableAsset(move.id); if (!item || move.busy) return;
    move.busy = true;
    return briefingPrompt('새 폴더 이름을 입력하세요.','새 폴더').then(function (name) {
      if (name == null || !String(name).trim()) return;
      if (!actionableAsset(move.id) || !validAssetDestination(item,move.folder)) throw new Error('저장 위치를 확인해 주세요.');
      move.busy = true;
      return writeOne('insuwork_items',{owner_id:currentUserId(),parent_id:move.folder || null,item_type:'folder',title:String(name).trim(),visibility:'private',legacy_payload:{workspace_category:assetCategory(item)}}).then(function (folder) { upsertWorkspaceItem(folder); move.folder = folder.id; });
    }).catch(saveError).finally(function () { move.busy = false; if (assetMoveState === move) assetMoveRender(); });
  }
  document.addEventListener('pointerdown',function (event) { var menu = document.getElementById('iw-asset-action-menu'); if (menu && !menu.contains(event.target) && !event.target.closest('.iw-asset-more')) closeAssetActions(); });
  ...
  var periodFilters = {}, periodResults = {}, periodSignatures = {}, periodCare = { owner: '', rows: [], status: '' };
  function periodFilter(area) { return periodFilters[area] || (periodFilters[area] = { basis: 'date', preset: 'all', start: '', end: '' }); }
  function periodRange(filter) {
    var today = ymd(new Date()), now = parseDate(today), year = now.getFullYear(), month = now.getMonth();
    if (filter.preset === 'today') return [today, today];
    if (filter.preset === 'week') return [addDays(today, -6), today];
    if (filter.preset === 'month') return [ymd(new Date(year, month, 1)), ymd(new Date(year, month + 1, 0))];
    if (filter.preset === 'lastMonth') return [ymd(new Date(year, month - 1, 1)), ymd(new Date(year, month, 0))];
    if (filter.preset === 'custom') return [filter.start, filter.end];
    return ['', ''];
  }
  function loadPeriodCare() {
    var owner = currentUserId(), cache = { owner: owner, rows: [], status: 'loading' }; periodCare = cache;
    function page(offset) {
      return api('insuwork_tasks?owner_id=eq.' + encodeURIComponent(owner) + '&deleted_at=is.null&legacy_source=eq.care_auto&order=id.asc&limit=500&offset=' + offset + '&select=id,customer_id,task_date,completed_at,legacy_source,legacy_id').then(function (rows) {
        cache.rows = cache.rows.concat(rows); return rows.length === 500 ? page(offset + 500) : null;
      });
    }
    return page(0).then(function () { cache.status = 'ready'; }).catch(function () { cache.status = 'error'; }).finally(function () { if (periodCare === cache) renderContent(); });
  }
  function setListPeriod(area, key, value) {
    if (!canEditCoverageTemplate() || ['customer', 'consultation'].indexOf(area) < 0) return;
    var filter = periodFilter(area);
    if (key === 'basis' && ['date', 'care', 'birthday'].indexOf(value) >= 0) filter.basis = area === 'consultation' ? 'date' : value;
    else if (key === 'preset' && ['today', 'week', 'month', 'lastMonth', 'all', 'custom'].indexOf(value) >= 0) { filter.preset = value; if (value === 'custom' && !filter.start) filter.start = filter.end = ymd(new Date()); }
    else if (key === 'start' || key === 'end') filter[key] = value;
    setKakaoBulkState(area, []);
    if (area === 'customer') { state.selectedCustomerDetail = null; state.customersRenderLimit = LIST_PAGE_SIZE; }
    else { state.selectedConsultation = null; state.consultationsRenderLimit = LIST_PAGE_SIZE; }
    if (filter.basis === 'care') loadPeriodCare();
    renderContent();
  }
  function periodRows(area, rows) {
    var filter = periodFilter(area), range = periodRange(filter), enabled = canEditCoverageTemplate();
    var signature = JSON.stringify([currentUserId(), filter, state.query, area === 'customer' ? state.customerNameQuery : state.consultNameQuery, area === 'customer' ? state.customerStatusFilter : state.consultationStatusFilter]);
    if (periodSignatures[area] !== signature) { setKakaoBulkState(area, []); periodSignatures[area] = signature; }
    function inside(date) { return !!date && (!range[0] || date >= range[0]) && (!range[1] || date <= range[1]); }
    if (enabled) rows = rows.filter(function (item) {
      if (filter.preset === 'custom' && (!range[0] || !range[1] || range[0] > range[1])) return false;
      if (area === 'customer' && filter.basis === 'care') {
        if (periodCare.owner !== currentUserId() || periodCare.status !== 'ready') return false;
        return periodCare.rows.some(function (event) { return String(event.customer_id) === String(item.id) && !event.completed_at && !obsoleteApplicationAnniversary(event) && inside(String(event.task_date || '').slice(0, 10)); });
      }
      if (filter.preset === 'all') return true;
      if (area === 'customer' && filter.basis === 'birthday') {
    ...
    setKakaoBulkState(area, kakaoBulkState(area).filter(function (id) { return periodResults[area].indexOf(id) >= 0; }));
    return rows;
  }
  function periodBarHtml(area, count) {
    if (!canEditCoverageTemplate()) return '';
    var filter = periodFilter(area), range = periodRange(filter);
    function change(key) { return 'OSInsuwork.setListPeriod(\'' + area + '\',\'' + key + '\',this.value)'; }
    var basis = area === 'customer' ? '<select aria-label="조회 기준" onchange="' + change('basis') + '">' + [['date', '최초 청약일'], ['care', '케어 예정일'], ['birthday', '생일']].map(function (entry) { return '<option value="' + entry[0] + '"' + (entry[0] === filter.basis ? ' selected' : '') + '>' + entry[1] + '</option>'; }).join('') + '</select>' : '<strong>등록일</strong>';
    var buttons = [['today', '오늘'], ['week', '최근 7일'], ['month', '이번 달'], ['lastMonth', '지난달'], ['all', '전체'], ['custom', '사용자 정의']].map(function (entry) { return '<button type="button" class="iw-btn' + (filter.preset === entry[0] ? ' primary' : '') + '" aria-pressed="' + (filter.preset === entry[0]) + '" onclick="OSInsuwork.setListPeriod(\'' + area + '\',\'preset\',\'' + entry[0] + '\')">' + entry[1] + '</button>'; }).join('');
    var dates = filter.preset === 'custom' ? '<input type="date" aria-label="조회 시작일" value="' + esc(filter.start) + '" onchange="' + change('start') + '"> ~ <input type="date" aria-label="조회 종료일" value="' + esc(filter.end) + '" onchange="' + change('end') + '">' : '';
    var note = filter.basis === 'care' ? (periodCare.status === 'loading' ? '케어 일정 조회 중…' : periodCare.status === 'error' ? '조회 실패. 케어 예정일을 다시 선택해 주세요.' : '미완료 케어 기준 · 같은 고객은 한 번만 표시') : '';
    if (filter.preset === 'custom' && (!range[0] || !range[1] || range[0] > range[1])) note = '시작일과 종료일을 확인해 주세요.';
    return '<div class="iw-period-bar">' + basis + buttons + dates + '<span>조회 ' + count + '명</span><small>' + esc(note || (range[0] ? range[0] + ' ~ ' + range[1] : '전체 기간')) + '</small></div>';
  }
  ...
  function kakaoBulkBarHtml(area, rows) {
    ...
    return '<div class="iw-kakao-bulkbar"><span>' + esc(areaLabel) + ' 선택 ' + selected.length + '명</span><button type="button" class="iw-btn" onclick="OSInsuwork.selectAllKakaoBulk(\'' + esc(area) + '\')">조회 결과 전체선택</button><button type="button" class="iw-btn" onclick="OSInsuwork.clearKakaoBulk(\'' + esc(area) + '\')">선택해제</button><button type="button" class="iw-btn iw-kakao-btn" onclick="OSInsuwork.openKakaoBulkDraft(\'' + esc(area) + '\')" ' + (selected.length ? '' : 'disabled') + '>카카오톡 발송하기</button></div>';
  }
  function customersHtml() {
    var columns = [{ key: 'date', label: '청약일자', width: 86 }, { key: 'name', label: '이름', width: 200 }, { key: 'birth', label: '생년월일', width: 92 }, { key: 'genderAge', label: '성별(보험나이)', width: 104 }, { key: 'phone', label: '전화번호', width: 116 }, { key: 'summary', label: '상담내용', width: 308, flex: true }, { key: 'status', label: '고객상태', width: 102 }];
    ...
    var periodHtml = periodBarHtml('customer', rows.length), bulkHtml = kakaoBulkBarHtml('customer', rows);
  ...
  function familySectionHtml(customer) {
    var members = familyMembers(customer), candidates = familyCandidates(customer);
    var memberRows = members.length > 1 ? members.map(function (member) {
      var self = String(member.id) === String(customer.id), relation = self ? '본인' : familyRelation(customer, member.id);
      return '<div class="iw-family-member"><button type="button" onclick="OSInsuwork.openFamilyMember(\'' + esc(member.id) + '\')"><strong>' + esc(member.name || '이름 없음') + '</strong><span>' + esc(relation) + '</span><small>' + esc(phoneText(member.phone || member.phone_raw || '') || '연락처 없음') + '</small></button>' + (self ? '' : '<button type="button" class="iw-family-remove" onclick="OSInsuwork.removeFamilyMember(\'' + esc(customer.id) + '\',\'' + esc(member.id) + '\')">가족에서 제외</button>') + '</div>';
    }).join('') : '<p class="iw-family-empty">연결된 가족이 없습니다.</p>';
    var currentGroupId = familyGroupId(customer);
    var options = state.data.customers.filter(function (entry) { return isManagedCustomer(entry) && String(entry.id) !== String(customer.id) && !(currentGroupId && familyGroupId(entry) === currentGroupId); }).sort(function (a, b) { return String(a.name || '').localeCompare(String(b.name || ''), 'ko'); }).map(function (entry) { return '<option value="' + esc(entry.id) + '">' + esc(entry.name || '이름 없음') + (phoneText(entry.phone || entry.phone_raw || '') ? ' · ' + esc(phoneText(entry.phone || entry.phone_raw || '')) : '') + '</option>'; }).join('');
    var candidateHtml = candidates.length ? '<div class="iw-family-candidates"><strong>가족 연결 후보</strong><p>상담내용에서 가족 표현과 이름이 함께 발견됐습니다. 확인 후 연결하세요.</p>' + candidates.map(function (entry) { return '<button type="button" onclick="OSInsuwork.prepareFamilyCandidate(\'' + esc(entry.id) + '\')">' + esc(entry.name || '이름 없음') + ' 연결 검토</button>'; }).join('') + '</div>' : '';
    var panelId = 'iwd-family-panel-' + customer.id;
    var summary = members.length > 1 ? '<small>연결 ' + members.length + '명</small>' : '<small>연결된 가족 없음</small>';
    var relations = '<option>배우자</option><option>부모</option><option>자녀</option><option>형제자매</option><option>기타</option>';
    var newFamilyForm = '<div class="iw-family-new" id="iwd-family-new"><div class="iw-family-new-grid"><label><span>관계</span><select id="iwd-family-new-relation">' + relations + '</select></label><label><span>이름</span><input id="iwd-family-new-name" autocomplete="off" placeholder="이름"></label><label><span>생년월일</span>' + dateMaskInputHtml('iwd-family-new-birth', '', '', 'aria-label="가족 생년월일"') + '</label><fieldset><legend>성별</legend><label><input type="radio" name="iwd-family-new-gender" value="남">남</label><label><input type="radio" name="iwd-family-new-gender" value="여">여</label></fieldset><label><span>전화번호</span><input id="iwd-family-new-phone" inputmode="numeric" autocomplete="off" placeholder="전화번호" oninput="OSInsuwork.formatConsultPhone(this)"></label><label><span>고객상태</span><select id="iwd-family-new-status"><option value="상태 확인 필요">상태 확인 필요</option><option>청약완료</option><option>철회</option><option>실효</option><option>부활</option></select></label></div><label class="iw-family-address-same"><input id="iwd-family-address-same" type="checkbox" checked onchange="OSInsuwork.toggleNewFamilyAddress(this)">현재 고객과 주소 동일</label><div class="iw-family-new-address" id="iwd-family-new-address" hidden><input id="iwd-family-new-zip" placeholder="우편번호"><input id="iwd-family-new-address-main" placeholder="주소"><input id="iwd-family-new-address-detail" placeholder="상세주소"></div><label class="iw-family-new-note"><span>메모</span><input id="iwd-family-new-note" placeholder="선택 입력"></label><div class="iw-family-new-actions"><button type="button" class="iw-btn primary" onclick="OSInsuwork.saveNewFamily(\'' + esc(customer.id) + '\')">등록 및 가족 연결</button></div></div>';
    var existingFamilyForm = '<div class="iw-family-existing" id="iwd-family-existing" hidden>' + candidateHtml + '<div class="iw-family-connect"><select id="iwd-family-customer" aria-label="연결할 기존 고객"><option value="">기존 고객 선택</option>' + options + '</select><select id="iwd-family-relation" aria-label="가족 관계">' + relations + '</select><button type="button" class="iw-btn" onclick="OSInsuwork.connectFamily(\'' + esc(customer.id) + '\')">가족 연결</button></div></div>';
    return '<section class="iw-family-section"><button type="button" class="iw-family-section-toggle" aria-expanded="false" aria-controls="' + esc(panelId) + '" onclick="OSInsuwork.toggleFamilySection(this,\'' + esc(panelId) + '\')"><span><strong>가족 추가</strong>' + summary + '</span><b aria-hidden="true">+</b></button>'
      + '<div class="iw-family-body" id="' + esc(panelId) + '" hidden><div class="iw-family-members">' + memberRows + '</div><div class="iw-family-modes" role="tablist"><button type="button" class="on" aria-selected="true" onclick="OSInsuwork.setFamilyAddMode(\'new\',this)">새 가족 등록</button><button type="button" aria-selected="false" onclick="OSInsuwork.setFamilyAddMode(\'existing\',this)">기존 고객 연결</button></div>' + newFamilyForm + existingFamilyForm + '</div></section>';
  }
  function toggleFamilySection(button, panelId) {
    var panel = document.getElementById(panelId); if (!panel) return;
    var opening = panel.hidden;
    panel.hidden = !opening;
    if (button) { button.setAttribute('aria-expanded', opening ? 'true' : 'false'); var icon = button.querySelector('b'); if (icon) icon.textContent = opening ? '−' : '+'; }
    if (opening) { var select = panel.querySelector('#iwd-family-new-relation'); if (select) select.focus({ preventScroll: true }); }
  }
  function setFamilyAddMode(mode, button) {
    var section = button && button.closest('.iw-family-section'); if (!section) return;
    var fresh = section.querySelector('#iwd-family-new'), existing = section.querySelector('#iwd-family-existing'), useExisting = mode === 'existing';
    if (fresh) fresh.hidden = useExisting; if (existing) existing.hidden = !useExisting;
    section.querySelectorAll('.iw-family-modes button').forEach(function (entry) { var on = entry === button; entry.classList.toggle('on', on); entry.setAttribute('aria-selected', on ? 'true' : 'false'); });
    var first = section.querySelector(useExisting ? '#iwd-family-customer' : '#iwd-family-new-relation'); if (first) first.focus({ preventScroll: true });
  }
  function toggleNewFamilyAddress(checkbox) { var box = document.getElementById('iwd-family-new-address'); if (box) box.hidden = !!(checkbox && checkbox.checked); }
  ...
  function consultationsHtml() {
    ...
    var periodHtml = periodBarHtml('consultation', rows.length), bulkHtml = kakaoBulkBarHtml('consultation', rows);
    ...
    return '<div class="iw-consult-screen">' + statusHtml() + '<div class="iw-toolbar"><h2>상담관리' + helpBadgeHtml('consultations') + '</h2></div>' + stats + (periodHtml ? '<div class="iw-management-toolbar">' + periodHtml + bulkHtml + '</div>' : bulkHtml) + favoritesFabHtml() + '<div class="iw-consult-layout' + (selected ? ' has-detail' : '') + '"><section class="iw-consult-master">' + list + '</section>' + detail + '</div></div>';
  }
  ...
  function eventPriority(event) { return event && event.event_type === 'holiday' ? 0 : event && event.event_type === 'term' ? 1 : event && event.event_type === 'memorial' ? 2 : event && event.event_type === 'birthday' ? 3 : event && event.event_type === 'insurance-age' ? 4 : event && event.event_type === 'application' ? 5 : isCareTask(event) ? 6 : 7; }
  ...
  function calendarEventKind(event) { return isCareTask(event) || (event && event.event_type === 'application') ? 'customer' : event && event.event_type === 'holiday' ? 'holiday' : event && event.event_type === 'term' ? 'term' : event && event.event_type === 'memorial' ? 'memorial' : event && event.event_type === 'birthday' ? 'birthday' : event && event.event_type === 'insurance-age' ? 'insurance-age' : 'schedule'; }
  ...
  function builtinCalendarChip(event) {
    var action = event && (event.event_type === 'insurance-age' || event.event_type === 'birthday' || event.event_type === 'application') && event.customer_id ? ' onclick="event.stopPropagation(); OSInsuwork.openCustomerFromEvent(\'' + esc(event.customer_id) + '\')"' : '';
  ...
  function birthdaySummaryChip(events, date) {
    if (!events.length) return '';
    return '<i class="birthday birthday-more" role="button" tabindex="0" title="생일 고객 전체 보기"' + calendarSummaryAttrs(events) + ' onclick="event.stopPropagation();OSInsuwork.openCalendarDay(\'' + esc(date) + '\')" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();event.stopPropagation();OSInsuwork.openCalendarDay(\'' + esc(date) + '\')}">생일 ' + events.length + '명</i>';
  }
  ...
  function applicationSummaryChip(events, date) {
    if (!events.length) return '';
    return '<i class="customer customer-more" role="button" tabindex="0" title="청약 고객 전체 보기"' + calendarSummaryAttrs(events) + ' onclick="event.stopPropagation();OSInsuwork.openCalendarDay(\'' + esc(date) + '\')" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();event.stopPropagation();OSInsuwork.openCalendarDay(\'' + esc(date) + '\')}">청약 ' + events.length + '명</i>';
  }
  ...
  function monthCalendarChips(events, date) {
    ...
    var birthdayEvents = builtIns.filter(function (event) { return event.event_type === 'birthday'; });
    var applicationEvents = builtIns.filter(function (event) { return event.event_type === 'application'; });
    var otherBuiltIns = builtIns.filter(function (event) { return event.event_type !== 'insurance-age' && event.event_type !== 'birthday' && event.event_type !== 'application'; });
    ...
    if (applicationEvents.length === 1) chips.push(careCalendarChip(applicationEvents[0]));
    else if (applicationEvents.length > 1) chips.push(applicationSummaryChip(applicationEvents, date));
    if (birthdayEvents.length === 1) chips.push(builtinCalendarChip(birthdayEvents[0]));
    else if (birthdayEvents.length > 1) chips.push(birthdaySummaryChip(birthdayEvents, date));
  ...
  function agendaView() {
    ...
    /* 같은 날짜 일정을 날짜별로 한 번만 묶어서 보여주고, 종류별 컬럼
       (공휴일·절기·기념일 | 일정 | 케어 | 상령일 | 생일)로 나눠 나란히 배치. 일/주/월 화면과 같은
    ...
    rows.forEach(function (event) {
      ...
      var colIndex = (kind === 'holiday' || kind === 'term' || kind === 'memorial') ? 0 : kind === 'customer' ? 2 : kind === 'insurance-age' ? 3 : kind === 'birthday' ? 4 : 1;
      byDate[date].cols[colIndex].push(event);
    });
    var header = '<div class="iw-agenda-header"><span></span><div class="iw-agenda-cols"><span>공휴일·절기·기념일</span><span>일정</span><span>케어</span><span>상령일</span><span>생일</span></div></div>';
  ...
  function sectionHtml() { if (state.section === 'ledger') return canEnterSection('ledger') && window.OSInsuworkLedger ? window.OSInsuworkLedger.html() : '<p>임태성 전용 화면입니다.</p>';
    ...
    if (state.section === 'insurance-portal') return canEnterSection('insurance-portal') && window.OSInsurancePortal ? window.OSInsurancePortal.html() : '<div class="iw-empty">보험포털에 접근할 수 없습니다.</div>';
    if (state.section === 'assets') return assetsHtml();
    if (state.section === 'coverage-analysis') return coverageAnalysisPageHtml();
    if (state.section === 'coverage-sheet') return window.OSInsuworkCoverageSheet ? window.OSInsuworkCoverageSheet.sectionHtml() : '<div class="iw-empty">보장분석 엑셀 편집기를 불러오지 못했습니다.</div>';
  ...
  var adminUsersRefreshTimer = 0, adminUsersEntryTimer = 0, adminUsersRefreshOwner = '', adminUsersUpdatedAt = '';
  function adminUsersRefreshVisible() {
    var view = document.getElementById('v-insuwork');
    return state.section === 'admin-users' && !document.hidden && view && view.classList.contains('on') && canSeeAdminUsers() && authenticated();
  }
  function stopAdminUsersRefresh() {
    window.clearInterval(adminUsersRefreshTimer); window.clearTimeout(adminUsersEntryTimer);
    adminUsersRefreshTimer = 0; adminUsersEntryTimer = 0; adminUsersRefreshOwner = '';
  }
  function syncAdminUsersRefresh() {
    if (!adminUsersRefreshVisible()) { stopAdminUsersRefresh(); return; }
    var owner = currentUserId();
    if (adminUsersRefreshTimer && adminUsersRefreshOwner === owner) return;
    stopAdminUsersRefresh(); adminUsersRefreshOwner = owner;
    adminUsersEntryTimer = window.setTimeout(function () { if (adminUsersRefreshVisible()) loadAdminUsers(true); }, 0);
    adminUsersRefreshTimer = window.setInterval(function () {
      if (!adminUsersRefreshVisible()) { stopAdminUsersRefresh(); return; }
      loadAdminUsers(true);
    }, 30000);
  }
  document.addEventListener('visibilitychange', syncAdminUsersRefresh);
  window.addEventListener('pagehide', stopAdminUsersRefresh);
  window.addEventListener('pageshow', syncAdminUsersRefresh);
  function loadAdminUsers(force) {
    ...
    return usersRequest
      ...
      .finally(function () {
        ...
        if (owner !== currentUserId() || !authenticated() || !canSeeAdminUsers()) { state.adminUsers = null; adminUsersUpdatedAt = ''; return; }
        if (state.section !== 'admin-users' || document.hidden || state.adminUserComposing) return;
        var input = document.getElementById('iw-admin-user-search'), main = document.getElementById('iw-main'), table = main && main.querySelector('.iw-admin-users-table');
        var scrollTop = main ? main.scrollTop : 0, tableLeft = table ? table.scrollLeft : 0, tableTop = table ? table.scrollTop : 0;
        if (input) { window.clearTimeout(state.adminUserTimer); state.adminUserQuery = input.value; }
        if (document.activeElement && document.activeElement.tagName === 'SELECT' && main && main.contains(document.activeElement)) return;
        renderContent();
  ...
  /* 오피스 서브플로우 3섹션은 전역 사이드 메뉴(navHtml)가 필요 없다 — renderShell()에서
     이 섹션들일 때는 navHtml() 자체를 렌더하지 않는다(PR #2113 후속: 이중 사이드 메뉴 수정). */
  var OFFICE_SECTIONS = ['personal-office', 'my-work', 'admin-users'];
  ...
  function renderShell() {
    ...
    var officeMode = OFFICE_SECTIONS.indexOf(state.section) >= 0;
    view.innerHTML = '<div class="iw-shell' + (STANDALONE ? ' iw-shell-compact' : '') + '">' + head + '<div class="iw-body' + (officeMode ? ' iw-body-office' : '') + '">' + (officeMode ? '' : navHtml()) + '<main class="iw-main" id="iw-main"></main></div></div><dialog class="iw-dialog" id="iw-dialog"><button class="iw-dialog-close" onclick="OSInsuwork.closeDialog()" aria-label="닫기">×</button><div id="iw-dialog-body"></div></dialog>'
  ...
  function scheduleSearch(value) { window.clearTimeout(state.searchTimer); state.searchTimer = window.setTimeout(function () { state.query = value; loadBriefingSearch(value); if (state.query.trim() && !state.fullLoaded) loadData(true); else renderContent(); }, 180); }
  function setUrl(push) { var url = '?view=insuwork&section=' + encodeURIComponent(state.section); if (state.section === 'insurance-portal') { var portalParams = new URLSearchParams(location.search); ['article','topic'].forEach(function (key) { if (portalParams.get(key)) url += '&' + key + '=' + encodeURIComponent(portalParams.get(key)); }); } if (state.section === 'daily-briefing') { var briefingParams = new URLSearchParams(location.search); ['article', 'briefingDate'].forEach(function (key) { if (briefingParams.get(key)) url += '&' + key + '=' + encodeURIComponent(briefingParams.get(key)); }); } if (state.section === 'calendar') url += '&mode=' + state.calendarMode + '&date=' + state.selectedDate; if (state.section === 'home') url += '&date=' + state.homeDate; if (state.section === 'tools') url += '&tool=' + encodeURIComponent(state.toolMode || 'calculator'); try { history[push ? 'pushState' : 'replaceState']({ view: 'insuwork', section: state.section }, '', url); } catch (_) {} }
  ...
  function storagePreviewError(error) {
    closePreview();
  ...
  function hydrateRichStorage() {
    var nodes = document.querySelectorAll('#v-insuwork [data-storage-path]');
    Array.prototype.forEach.call(nodes, function (node) {
      var path = node.getAttribute('data-storage-path'); if (!path) return;
      node.onclick = function (event) { if (event) event.preventDefault(); openStoragePreview(node); };
      if (node.tagName === 'IMG') { node.classList.add('iw-previewable'); node.title = '클릭하면 크게 보기'; }
      signStoragePath(path).then(function (url) { if (node.tagName === 'IMG') node.src = url; else node.href = url; prepareFileDrag(node, url); }).catch(function () {});
    });
  }
  function loadPdfJs() {
  ...
  function showEvent(id) {
    ...
    if ((event.event_type === 'insurance-age' || event.event_type === 'birthday' || event.event_type === 'application') && event.customer_id) { openCustomerFromEvent(event.customer_id); return; }
  ...
  function deleteAssetFolder(id) {
    ...
    briefingConfirm('“' + String(folder.title || '폴더') + '” 폴더를 삭제할까요?', '폴더 삭제', '삭제', true).then(function (ok) {
      ...
      softDelete('insuwork_items?id=eq.' + encodeURIComponent(id) + '&owner_id=eq.' + encodeURIComponent(currentUserId()))
        .then(function () { removeWorkspaceItemsLocal([id]); if (String(state.assetFolder) === String(id)) state.assetFolder = null; state.assetFilter = category; renderContent(); if (typeof window.toast === 'function') window.toast('폴더를 삭제했습니다.'); }).catch(saveError);
    });
  }
  ...
  function uploadCoverageSources(files, upload) {
    if (!files) return Promise.resolve(null);
    var sources = [];
    return (Array.isArray(files) ? files : [files]).reduce(function (job, file) { return job.then(function () { return upload(file); }).then(function (source) { sources.push({ id: source.id, name: source.title }); }); }, Promise.resolve()).then(function () {
      return { id: sources[0].id, title: sources.map(function (source) { return source.name; }).join(' · '), files: sources };
    });
  }
  ...
  function persistCoverageWorkspaceAnalysis(record, sourceFile, asTemplate) {
    ...
    return ready.then(function (saved) { upsertWorkspaceItem(saved); if (!sourceFile) return next; return uploadCoverageSources(sourceFile, function (file) { return uploadCoverageWorkspaceSource(rootId, file); }).then(function (source) { next.sourceItemId = source.id; next.source = Object.assign({}, next.source || {}, { name: source.title, itemId: source.id, files: source.files }); return updateOne('insuwork_items?id=eq.' + encodeURIComponent(rootId) + '&owner_id=eq.' + encodeURIComponent(currentUserId()), { body: coverageAnalysisSummary(next), legacy_payload: payload }).then(function (updated) { upsertWorkspaceItem(updated); return next; }); }); }).then(function (savedRecord) { if (asTemplate && typeof window.toast === 'function') window.toast('보장분석 기본 양식을 저장했습니다.'); return savedRecord; });
  }
  function saveCoverageWorkspaceToCustomer(record) {
    ...
    return saveCoverageAnalysis(customerId, record, null, null).then(function (saved) { if (typeof window.toast === 'function') window.toast('선택한 고객 카드에 보장분석·보험비교를 저장했습니다.'); return saved; });
  }
  function customerCoveragePdfFiles(customerId) {
    var roots = new Set((state.data.items || []).filter(function (entry) {
      var p = entry.legacy_payload || {};
      if (entry.owner_id !== currentUserId() || entry.deleted_at) return false;
      if (String(p.customer_id || '') === String(customerId)) return true;
      return p.attachment_root && p.consultation_id && (state.data.consultations || []).some(function (c) { return String(c.id) === String(p.consultation_id) && String(c.customer_id) === String(customerId); });
    }).map(function (entry) { return String(entry.id); }));
    return (state.data.items || []).filter(function (file) {
      var p = file.legacy_payload || {};
      return file.owner_id === currentUserId() && !file.deleted_at && file.storage_path && (/pdf/i.test(file.mime_type || '') || String(file.extension || '').toLowerCase() === 'pdf') && (String(p.customer_id || '') === String(customerId) || roots.has(String(file.parent_id || '')));
    });
  }
  function openCustomerCoverage(customerId, fileId) {
  ...
  function openCoverageAttachmentPicker(customerId, preselectedId) {
    if (!canUseCoverageAnalysis() || !getCoverageCustomerInfo(customerId)) return;
    var files = customerCoveragePdfFiles(customerId), customer = getCoverageCustomerInfo(customerId);
    var choices = files.map(function (file) { return '<label style="display:flex;gap:10px;align-items:center;margin:10px 0"><input type="checkbox" name="coverage-pdf" value="' + esc(file.id) + '"' + (preselectedId === file.id || files.length === 1 ? ' checked' : '') + '><span>' + esc(file.title || '보장분석.pdf') + '</span></label>'; }).join('');
    dialog('<div class="iw-form"><h2>' + esc(customer.name || '고객') + ' · 자료 추가 분석</h2><p>분석할 PDF를 선택하세요. 기존 표에 보완하며 원본 파일은 그대로 연결합니다.</p><div id="iw-ca-attachment-choices">' + (choices || '<p>첨부된 PDF가 없습니다. 보장분석 화면의 파일 불러오기를 이용해 주세요.</p>') + '</div><div class="iw-form-actions"><button type="button" class="iw-btn" onclick="OSInsuwork.closeDialog()">닫기</button>' + (files.length ? '<button type="button" class="iw-btn primary" onclick="OSInsuwork.analyzeCoverageAttachments(\'' + esc(customerId) + '\')">선택 자료 분석</button>' : '') + '</div></div>');
  }
  function analyzeCoverageAttachments(customerId) {
    if (!canUseCoverageAnalysis()) return;
    var allowed = new Set(customerCoveragePdfFiles(customerId).map(function (file) { return String(file.id); }));
    var ids = Array.from(document.querySelectorAll('#iw-ca-attachment-choices input:checked')).map(function (input) { return input.value; });
    if (!ids.length || ids.some(function (id) { return !allowed.has(id); })) return coverageError('분석할 고객 PDF를 선택해 주세요.');
    forceCloseDialog();
    return window.OSInsuworkCoverage.importCustomerPdfs(customerId, ids).then(function () { coverageNotice('분석한 작업표를 저장했습니다. 정리 후 고객에게 저장해 주세요.'); }).catch(function (error) { coverageError(error.message || String(error)); });
  }
  function coverageAnalysisSectionHtml(customerId) {
    if (!canUseCoverageAnalysis() || !window.OSInsuworkCoverage || !customerId) return '';
    var item = coverageAnalysisItem(customerId), saved = item && item.legacy_payload && item.legacy_payload.coverage_analysis;
    return '<section><h3>보장분석·보험비교</h3><p>' + (saved ? esc(coverageAnalysisSummary(saved)) : '첨부 PDF로 고객 보장을 분석하고 비교표를 정리합니다.') + '</p><button type="button" class="iw-btn" onclick="OSInsuwork.openCustomerCoverage(\'' + esc(customerId) + '\')">' + (saved ? '보장분석 이어서 보기' : '보장분석 시작') + '</button></section>';
  }
  ...
  function saveCoverageAnalysis(customerId, record, sourceFile, existingSourceId) {
    ...
    var body = { owner_id: currentUserId(), item_type: 'memo', title: '보장분석·보험비교 · ' + (customer.name || '고객'), body: coverageAnalysisSummary(next), visibility: 'private', legacy_payload: { workspace_category: 'customer', customer_id: customerId, coverage_analysis_record: true, coverage_analysis: next } };
  ...
  function saveCoverageSheetWorkbook(workbookBase64, fileName) {
    if (!canUseCoverageAnalysis()) return Promise.reject(new Error('보장분석 엑셀을 저장할 권한이 없습니다.'));
    if (!authenticated()) return Promise.reject(new Error('로그인한 뒤 저장해 주세요.'));
    workbookBase64 = String(workbookBase64 || '');
    if (!workbookBase64 || workbookBase64.length > 8 * 1024 * 1024) return Promise.reject(new Error('작업본이 비어 있거나 저장 가능한 크기를 초과했습니다.'));
    var owner = currentUserId(), now = new Date().toISOString(), safeName = String(fileName || '보장분석 작업본.xlsx').slice(0, 120);
    return api('insuwork_items?owner_id=eq.' + encodeURIComponent(owner) + '&item_type=eq.memo&deleted_at=is.null&legacy_payload->>setting_key=eq.coverage_sheet_workbook&order=updated_at.desc&limit=1&select=*').then(function (rows) {
      var existing = Array.isArray(rows) && rows[0] || null;
      var body = { owner_id: owner, item_type: 'memo', title: '보장분석 엑셀 작업본', body: '브라우저에서 편집한 비공개 보장분석 엑셀 작업본입니다.', visibility: 'private', legacy_payload: { workspace_category: 'settings', setting_key: 'coverage_sheet_workbook', workbook_base64: workbookBase64, file_name: safeName, updated_at: now } };
      return existing ? updateOne('insuwork_items?id=eq.' + encodeURIComponent(existing.id) + '&owner_id=eq.' + encodeURIComponent(owner) + '&deleted_at=is.null', body) : writeOne('insuwork_items', body);
    }).then(function (saved) { upsertWorkspaceItem(saved); return { id: saved.id, updatedAt: saved.updated_at || now }; });
  }
  ...
  function kakaoHistoryRows(area, id) {
    return (state.data.items || []).filter(function (entry) {
      ...
      if (payload.workspace_category !== 'kakao_message_pilot' || payload.target_area !== area) return false;
      if (area === 'customer') return String(payload.customer_id || '') === String(id || '');
      return String(payload.consultation_id || '') === String(id || '');
    }).sort(function (a, b) { return String(b.created_at || '').localeCompare(String(a.created_at || '')); }).slice(0, 5);
  }
  function kakaoHistoryHtml(area, id) {
    if (!canUseKakaoPilot()) return '';
    var rows = kakaoHistoryRows(area, id);
    if (!rows.length) return '<section class="iw-kakao-history"><h3>카카오 발송 준비 이력</h3><p>아직 저장된 발송 준비 기록이 없습니다.</p></section>';
    return '<section class="iw-kakao-history"><h3>카카오 발송 준비 이력</h3><div>' + rows.map(function (entry) {
      var payload = entry.legacy_payload || {}, text = stripHtml(entry.body || '');
      return '<article><strong>' + esc(payload.template_label || entry.title || '카카오톡') + '</strong><span>' + esc(formatDate(entry.created_at)) + ' · ' + esc(payload.send_status === 'draft' ? '발송 준비' : payload.send_status || '기록') + '</span><p>' + esc(text).slice(0, 180) + '</p></article>';
    }).join('') + '</div></section>';
  }
  function kakaoHubRows() {
    return (state.data.items || []).filter(isKakaoPilotItem).sort(function (a, b) { return String(b.created_at || '').localeCompare(String(a.created_at || '')); });
  }
  function kakaoHubHtml() {
    if (!canUseKakaoPilot()) return '';
    var rows = kakaoHubRows(), pending = rows.filter(function (entry) { return (entry.legacy_payload || {}).send_status === 'draft'; }).length;
    var failed = rows.filter(function (entry) { var status = (entry.legacy_payload || {}).send_status; return status === 'failed' || status === 'needs_attention'; }).length;
    var unread = 0, actionCount = unread + failed + pending, tone = unread || failed ? ' danger' : (pending ? ' waiting' : '');
    var list = rows.slice(0, 8).map(function (entry) {
      var payload = entry.legacy_payload || {}, area = payload.target_area === 'customer' ? '계약관리' : '상담관리';
      return '<article><div><strong>' + esc(payload.recipient_name || '고객') + '</strong><span>' + esc(area + ' · ' + (payload.template_label || '카카오톡')) + '</span></div><small>' + esc(formatDate(entry.created_at)) + '</small></article>';
    }).join('') || '<p>아직 카카오 발송 준비 기록이 없습니다.</p>';
    return '<div class="iw-kakao-hub"><button type="button" class="iw-kakao-hub-toggle' + tone + '" onclick="OSInsuwork.toggleKakaoHub(event)" aria-haspopup="true" aria-expanded="false" title="카카오 업무함" aria-label="카카오 업무함 열기"><img src="/dayfolder/app/assets/dayfolder-routine-office-greeting-q71AaR6m.png" alt="">' + (actionCount ? '<b>' + actionCount + '</b>' : '') + '</button><section class="iw-kakao-hub-panel" id="iw-kakao-hub-panel" hidden><header><strong>카카오 업무함</strong><button type="button" onclick="OSInsuwork.toggleKakaoHub(event,false)" aria-label="닫기">×</button></header><div class="iw-kakao-hub-stats"><span><b>' + unread + '</b>새 문의</span><span><b>' + pending + '</b>발송 대기</span><span><b>' + failed + '</b>확인 필요</span></div><div class="iw-kakao-hub-tabs"><button type="button" class="on">전체</button><button type="button">알림톡</button><button type="button">친구톡</button><button type="button">상담톡</button></div><div class="iw-kakao-hub-list">' + list + '</div></section></div>';
  }
  function toggleKakaoHub(event, force) {
    if (event) event.stopPropagation();
    var panel = document.getElementById('iw-kakao-hub-panel'), toggle = document.querySelector('#v-insuwork .iw-kakao-hub-toggle');
    if (!panel) return;
  ...
  var customerAddressSearchTimers = {};
  ...
  function searchCustomerAddress(idPrefix, query, forceOpen) {
    ...
    var q = String(query == null ? value(prefix + 'address') : query).trim();
    if (!wrap.hidden && !forceOpen) { closeCustomerAddress(id); return; }
    function showPostcode() {
      try {
        wrap.hidden = false;
        container.innerHTML = '';
        new window.daum.Postcode({
          width: '100%', height: '100%', autoClose: false, maxSuggestItems: 6,
          onresize: function (size) { if (size && size.height) container.style.height = Math.min(Math.max(size.height, 250), 340) + 'px'; },
        ...
        }).embed(container, { q: q, autoClose: false });
  ...
  function toggleEventTime() {
    ...
    if (row.hasAttribute('hidden')) {
      ...
      if (toggle) toggle.textContent = '시간 제거';
      var allDay = document.getElementById('iwf-event-all-day'); if (allDay) allDay.focus();
      return;
    }
    row.setAttribute('hidden', '');
    var allDay = document.getElementById('iwf-event-all-day');
    if (allDay && allDay.checked) { allDay.checked = false; toggleEventAllDay(allDay); }
    if (toggle) { toggle.textContent = '+ 시간 추가'; toggle.focus(); }
  }
  function toggleEventAllDay(input) {
    var row = document.getElementById('iwf-event-timerow'); if (!row) return;
    row.classList.toggle('is-all-day', !!(input && input.checked));
    Array.prototype.forEach.call(row.querySelectorAll('.iw-event-timegroup select'), function (select) { select.disabled = !!(input && input.checked); });
  }
  function eventFormHtml(event, customerId) {
    ...
    return '<input id="iwf-event-title" class="iw-event-title-input" required autocomplete="off" placeholder="제목 추가" value="' + esc(event ? event.title || '' : '') + '">'
      + '<div class="iw-event-datebar"><input id="iwf-event-date" type="date" required value="' + esc(startDate) + '"><span class="iw-event-sep">–</span><input id="iwf-event-end-date" type="date" required value="' + esc(endDate) + '"><button type="button" class="iw-event-time-toggle" id="iwf-event-time-toggle" onclick="OSInsuwork.toggleEventTime()">' + (hasTime ? '시간 제거' : '+ 시간 추가') + '</button></div>'
      + '<div class="iw-event-timerow" id="iwf-event-timerow"' + (hasTime ? '' : ' hidden') + '><label class="iw-event-all-day"><input id="iwf-event-all-day" type="checkbox" onchange="OSInsuwork.toggleEventAllDay(this)"><span>종일</span></label><div class="iw-event-time-fields">' + timePartsHtml('iwf-event-time', startTime, '시작') + '<span class="iw-event-sep">–</span>' + timePartsHtml('iwf-event-end-time', endTime, '종료') + '</div></div>'
  ...
  function toolsCaptureHtml() {
    return '<section class="iw-tool-workspace iw-tool-capture-page"><div class="iw-tool-pane iw-capture-start"><h3>채팅창 스크롤 캡처</h3><p>캡처할 채팅 서비스를 선택하세요. 지정한 사이트의 채팅 탭으로 이동해 저장할 영역을 위에서 아래로 드래그합니다.</p><div class="iw-capture-sites"><button type="button" class="iw-btn primary iw-capture-button" onclick="OSInsuwork.startChatScrollCapture(\'happytalk\')">해피톡 캡처</button><button type="button" class="iw-btn primary iw-capture-button" onclick="OSInsuwork.startChatScrollCapture(\'bomapp\')">보맵 캡처</button></div><p class="iw-capture-status" id="iw-capture-status" aria-live="polite">캡처할 채팅 화면을 먼저 열어 주세요.</p></div><div class="iw-tool-pane iw-capture-install"><h3>처음 한 번만 설치</h3><ol><li><a class="iw-capture-download" href="/downloads/insurance-work-scroll-capture.zip?v=1.0.5" download>확장 프로그램 내려받기</a> 후 압축을 풉니다.</li><li>크롬 주소창에 <code>chrome://extensions</code>를 입력합니다.</li><li><strong>개발자 모드</strong>를 켜고 <strong>압축해제된 확장 프로그램을 로드합니다</strong>.</li><li>압축을 푼 폴더를 선택한 뒤 해피톡·보맵플래너 탭을 한 번 새로고침합니다.</li></ol><small>캡처 이미지는 서버에 전송하지 않고 사용자의 PC에 PNG로 저장합니다.</small></div></section>';
  }
  function setCaptureStatus(message, kind) {
    var status = document.getElementById('iw-capture-status');
    var buttons = document.querySelectorAll('.iw-capture-button');
    if (status) { status.textContent = message; status.className = 'iw-capture-status' + (kind ? ' ' + kind : ''); }
    Array.prototype.forEach.call(buttons, function (button) { button.disabled = kind === 'working'; });
  }
  function startChatScrollCapture(target) {
    if (!canSeeScrollCapture()) return;
    var siteName = target === 'happytalk' ? '해피톡' : '보맵';
    setCaptureStatus(siteName + ' 채팅 탭을 확인하고 있습니다…', 'working');
    var settled = false;
    var events = {
      'insuwork-scroll-capture:started': ['채팅 화면으로 이동했습니다. 캡처할 영역을 드래그하세요.', 'ok'],
      'insuwork-scroll-capture:no-target': ['열려 있는 ' + siteName + ' 채팅 탭이 없습니다.', 'error'],
      'insuwork-scroll-capture:reload-target': [siteName + ' 채팅 탭을 한 번 새로고침한 뒤 다시 시작해 주세요.', 'error'],
    ...
    window.dispatchEvent(new CustomEvent('insuwork-scroll-capture:start', { detail: { target: target } }));
  ...
  function storedBusinessCardSetting() { try { return JSON.parse(localStorage.getItem('iw_business_card_setting') || 'null'); } catch (_) { return null; } }
  function rememberBusinessCardSetting(card) { try { if (card && card.storage_path) localStorage.setItem('iw_business_card_setting', JSON.stringify(card)); } catch (_) {} }
  function businessCardSetting() { return (state.data.items || []).find(isBusinessCardSetting) || storedBusinessCardSetting(); }
  function businessCardUploadHtml() {
    var card = businessCardSetting();
    return '<div class="iw-kakao-card-upload" id="iwf-kakao-card-upload" hidden><div class="iw-kakao-card-info"><strong>명함 이미지</strong><span id="iwf-kakao-card-status">' + (card ? esc(card.title || '등록된 명함') : '등록된 명함 없음') + '</span></div><label class="iw-btn">명함 이미지 업로드<input type="file" accept="image/*" hidden onchange="OSInsuwork.uploadKakaoBusinessCard(this)"></label><div class="iw-kakao-card-preview"><img id="iwf-kakao-card-preview-img" alt="" hidden><span id="iwf-kakao-card-empty">등록된 명함이 없습니다.</span></div><p>명함 템플릿 선택 시 사용할 기본 명함으로 저장됩니다. 실제 카카오 발송에서는 승인된 버튼 링크로 연결합니다.</p></div>';
  }
  function setKakaoBusinessCardPreview(src) {
    var img = document.getElementById('iwf-kakao-card-preview-img'), empty = document.getElementById('iwf-kakao-card-empty');
    if (!img || !empty) return;
    if (src) { img.src = src; img.hidden = false; empty.hidden = true; }
    else { img.removeAttribute('src'); img.hidden = true; empty.hidden = false; }
  }
  function renderKakaoBusinessCardPreview(card) {
    setKakaoBusinessCardPreview('');
    if (!card || !card.storage_path) return;
    signStoragePath(card.storage_path).then(setKakaoBusinessCardPreview).catch(function () {});
  }
  function syncKakaoTemplateExtras() {
    var select = document.getElementById('iwf-kakao-template'), panel = document.getElementById('iwf-kakao-card-upload'), status = document.getElementById('iwf-kakao-card-status');
    if (!select || !panel) return;
    var showCard = select.value === 'name_card';
    panel.hidden = !showCard;
    var form = panel.closest('.iw-kakao-form');
    if (form) form.classList.toggle('has-card', showCard);
    if (status) { var card = businessCardSetting(); status.textContent = card ? (card.title || '등록된 명함') : '등록된 명함 없음'; renderKakaoBusinessCardPreview(card); }
  }
  function uploadKakaoBusinessCard(input) {
    var file = input && input.files && input.files[0], status = document.getElementById('iwf-kakao-card-status');
    if (input) input.value = '';
    if (!file) return;
    if (!/^image\//.test(file.type || '')) { if (status) status.textContent = '이미지 파일만 업로드할 수 있습니다.'; return; }
    if (file.size > 8 * 1024 * 1024) { if (status) status.textContent = '8MB 이하 이미지만 업로드해 주세요.'; return; }
    if (!authenticated() || !window.db || !window.db.getToken()) { if (status) status.textContent = '로그인 후 업로드할 수 있습니다.'; return; }
    setKakaoBusinessCardPreview(URL.createObjectURL(file));
    if (status) status.textContent = '업로드 중입니다.';
    var owner = currentUserId(), id = crypto.randomUUID(), dot = file.name.lastIndexOf('.'), ext = dot > 0 ? file.name.slice(dot + 1).toLowerCase().replace(/[^a-z0-9]/g, '') : '', path = owner + '/profile/business-card-' + id + (ext ? '.' + ext : '');
    fetch(window.db.url('/storage/v1/object/myspace/' + path.split('/').map(encodeURIComponent).join('/')), { method: 'POST', headers: { apikey: window.db.key, Authorization: 'Bearer ' + window.db.getToken(), 'Content-Type': file.type || 'application/octet-stream', 'x-upsert': 'false' }, body: file }).then(function (response) {
      ...
      var existing = businessCardSetting(), body = { owner_id: owner, item_type: 'file', title: file.name || '명함 이미지', storage_path: path, mime_type: file.type || null, extension: ext || null, file_size: file.size, visibility: 'private', legacy_payload: { workspace_category: 'settings', setting_key: 'business_card' } };
      return existing ? updateOne('insuwork_items?id=eq.' + encodeURIComponent(existing.id) + '&owner_id=eq.' + encodeURIComponent(owner), body) : writeOne('insuwork_items', Object.assign({ id: id, created_at: new Date().toISOString() }, body));
    }).then(function (saved) {
      upsertWorkspaceItem(saved);
      rememberBusinessCardSetting(saved);
      if (status) status.textContent = (saved && saved.title) || '명함 이미지 등록 완료';
      syncKakaoTemplateExtras();
    }).catch(function (error) {
      console.warn(error);
      if (status) status.textContent = '업로드하지 못했습니다. 잠시 후 다시 시도해 주세요.';
    });
  }
  function kakaoDraftRow(target, template, bodyText) {
    ...
    return {
      ...
      legacy_payload: {
        ...
        business_card_item_id: card ? card.id : null,
        business_card_storage_path: card ? card.storage_path : null,
  ...
  function selectAllKakaoBulk(area) {
    if (!canUseKakaoPilot()) return;
    setKakaoBulkState(area, (periodResults[area] || []).slice());
    renderContent();
  }
  ...
  function kakaoBulkTargets(area) {
    ...
    return kakaoBulkState(area).filter(function (id) { return (periodResults[area] || []).indexOf(id) >= 0; }).map(function (id) { return kakaoTarget(kind, id); }).filter(Boolean);
  }
  function openKakaoBulkDraft(area) {
    ...
    dialog('<form class="iw-form iw-kakao-form" onsubmit="event.preventDefault();OSInsuwork.saveKakaoBulkDraft(\'' + esc(area) + '\')"><div class="iw-kakao-head"><span>카카오 파일럿</span><h2>카카오톡 발송하기</h2><p>' + esc(area === 'customer' ? '계약관리 기존 고객' : '상담관리 신규 상담 고객') + '에게 같은 템플릿을 발송 준비합니다.</p></div><div class="iw-kakao-compose"><div class="iw-kakao-message-pane"><div class="iw-kakao-summary"><strong>선택 ' + targets.length + '명</strong><span>' + (missingPhone ? '연락처 없음 ' + missingPhone + '명 포함' : '발송 준비 가능') + '</span></div><p class="iw-kakao-recipients">' + esc(names) + '</p><div class="iw-kakao-fields"><label><span>템플릿</span><select id="iwf-kakao-template" onchange="OSInsuwork.refreshKakaoBulkPreview()">' + options + '</select></label><label><span>발송 문안</span><textarea id="iwf-kakao-body" rows="7">' + esc(kakaoMessagePreview(templates[0], { name: '#{고객명}' })) + '</textarea></label></div><p class="iw-kakao-note">#{고객명}은 각 고객 이름으로 바뀌어 발송 준비 기록에 저장됩니다. 담당자명, 회사명, 연락처는 개인정보 수정의 내 정보 기준으로 채워집니다. 실제 알림톡 발송은 공급자 API 연결 후 가능합니다.</p></div>' + businessCardUploadHtml() + '</div><div class="iw-form-actions iw-kakao-actions"><button type="button" class="iw-btn" onclick="OSInsuwork.closeDialog()">취소</button><button type="submit" class="iw-btn primary">선택 ' + targets.length + '명 저장</button></div></form>');
    syncKakaoTemplateExtras();
  }
  ...
  function showSearchImageHover(event) {
    var card = event.currentTarget, source = card && card.querySelector('.iw-asset-preview img'), tip = document.getElementById('iw-search-image-hover');
    var expanded = card && card.closest('#v-insuwork.iw-expanded-images');
    if (expanded && (card.closest('.large') || !window.matchMedia('(hover:hover)').matches)) return;
    if (!source || !source.src || !tip) return;
    var image = document.createElement('img'); image.src = source.src; image.alt = source.alt || '';
    tip.replaceChildren(image); tip.style.display = 'flex'; tip.setAttribute('aria-hidden', 'false');
    var rect = card.getBoundingClientRect(), width = expanded ? Math.min(250, window.innerWidth - 28) : Math.min(420, Math.max(260, window.innerWidth * 0.3)), gap = 14;
    var left = rect.right + gap; if (left + width > window.innerWidth - gap) left = rect.left - width - gap;
  ...
  window.addEventListener('resize', hideSearchImageHover);
  function hideSearchImageHover() { var tip = document.getElementById('iw-search-image-hover'); if (tip) { tip.style.display = 'none'; tip.setAttribute('aria-hidden', 'true'); tip.replaceChildren(); } }
  ...
  function openFamilyMember(id) { forceCloseDialog(); state.selectedCustomerDetail = id; renderContent(); }
  function showFamilyGroup(id) {
    var customer = state.data.customers.find(function (entry) { return String(entry.id) === String(id); }); if (!customer) return;
    var members = familyMembers(customer); if (members.length < 2) return;
    dialog('<div class="iw-family-dialog"><h2>' + esc(customer.name || '고객') + ' 가족 · ' + members.length + '명</h2><div class="iw-family-members">' + members.map(function (member) { return '<button type="button" class="iw-family-dialog-member" onclick="OSInsuwork.openFamilyMember(\'' + esc(member.id) + '\')"><strong>' + esc(member.name || '이름 없음') + '</strong><span>' + esc(String(member.id) === String(customer.id) ? '본인' : familyRelation(customer, member.id)) + '</span><small>' + esc(phoneText(member.phone || member.phone_raw || '') || '연락처 없음') + '</small></button>'; }).join('') + '</div><div class="iw-form-actions"><button type="button" class="iw-btn" onclick="OSInsuwork.closeDialog()">닫기</button></div></div>');
  }
  function prepareFamilyCandidate(id) { var select = document.getElementById('iwd-family-customer'); if (select) { select.value = id; select.focus(); } }
  function saveFamilyProfiles(customers) {
    return Promise.all(customers.map(function (entry) { return updateOne('insuwork_customers?id=eq.' + encodeURIComponent(entry.id) + '&owner_id=eq.' + encodeURIComponent(currentUserId()), { profile: entry.profile }).then(function (saved) { upsertCustomer(saved); return saved; }); }));
  }
  function linkFamilyRecords(customer, target, relation) {
    var groupId = familyGroupId(customer) || familyGroupId(target) || crypto.randomUUID();
    var oldGroups = [familyGroupId(customer), familyGroupId(target)].filter(Boolean);
    var group = state.data.customers.filter(function (entry) { return String(entry.id) === String(customer.id) || String(entry.id) === String(target.id) || oldGroups.indexOf(familyGroupId(entry)) >= 0; });
    var currentRelations = Array.isArray(customerProfile(customer).family_relations) ? customerProfile(customer).family_relations.slice() : [];
    var targetRelations = Array.isArray(customerProfile(target).family_relations) ? customerProfile(target).family_relations.slice() : [];
    currentRelations = currentRelations.filter(function (entry) { return String(entry.customer_id) !== String(target.id); }); currentRelations.push({ customer_id: target.id, relation: relation });
    targetRelations = targetRelations.filter(function (entry) { return String(entry.customer_id) !== String(customer.id); }); targetRelations.push({ customer_id: customer.id, relation: reciprocalFamilyRelation(relation) });
    var updates = group.map(function (entry) { var profile = Object.assign({}, customerProfile(entry), { family_group_id: groupId }); if (String(entry.id) === String(customer.id)) profile.family_relations = currentRelations; if (String(entry.id) === String(target.id)) profile.family_relations = targetRelations; return { id: entry.id, profile: profile }; });
    return saveFamilyProfiles(updates);
  }
  function connectFamily(customerId) {
    var customer = state.data.customers.find(function (entry) { return String(entry.id) === String(customerId); }), targetId = value('iwd-family-customer'), relation = value('iwd-family-relation') || '기타';
    var target = state.data.customers.find(function (entry) { return String(entry.id) === String(targetId); });
    if (!customer || !target || String(customer.id) === String(target.id)) { briefingAlert('연결할 기존 고객을 선택해 주세요.', '가족 연결'); return; }
    linkFamilyRecords(customer, target, relation).then(function () { renderContent(); if (typeof window.toast === 'function') window.toast(customer.name + ' 고객과 ' + target.name + ' 고객을 가족으로 연결했습니다.'); }).catch(saveError);
  }
  function saveNewFamily(customerId) {
    var customer = state.data.customers.find(function (entry) { return String(entry.id) === String(customerId); }); if (!customer) return;
    var name = value('iwd-family-new-name'), birth = value('iwd-family-new-birth'), phone = phoneText(value('iwd-family-new-phone')), relation = value('iwd-family-new-relation') || '기타', status = value('iwd-family-new-status') || '상태 확인 필요', note = value('iwd-family-new-note');
    var genderInput = document.querySelector('input[name="iwd-family-new-gender"]:checked'), gender = genderInput ? genderInput.value : '';
    if (!name) { briefingAlert('가족 이름을 입력해 주세요.', '새 가족 등록'); return; }
    var duplicate = state.data.customers.find(function (entry) { if (!isManagedCustomer(entry) || String(entry.id) === String(customer.id)) return false; var profile = customerProfile(entry); return (phone && digits(entry.phone || entry.phone_raw || '') === digits(phone)) || (birth && searchNorm(entry.name) === searchNorm(name) && String(profile.birth_date || '').slice(0, 10) === birth); });
    if (duplicate) {
      briefingConfirm(duplicate.name + ' 고객과 정보가 같습니다. 새로 등록하지 않고 기존 고객을 가족으로 연결할까요?', '중복 고객 확인', '기존 고객 연결').then(function (ok) { if (!ok) return; return linkFamilyRecords(customer, duplicate, relation).then(function () { renderContent(); if (typeof window.toast === 'function') window.toast(duplicate.name + ' 기존 고객을 가족으로 연결했습니다.'); }); }).catch(saveError);
      return;
    }
  ...
  function removeFamilyMember(customerId, memberId) {
    var customer = state.data.customers.find(function (entry) { return String(entry.id) === String(customerId); }), member = state.data.customers.find(function (entry) { return String(entry.id) === String(memberId); });
    if (!customer || !member || !familyGroupId(customer) || familyGroupId(customer) !== familyGroupId(member)) return;
    briefingConfirm(member.name + ' 고객을 이 가족에서 제외할까요? 고객 기록은 삭제되지 않습니다.', '가족에서 제외', '제외').then(function (ok) {
      if (!ok) return;
      var remaining = familyMembers(customer).filter(function (entry) { return String(entry.id) !== String(member.id); });
      var updates = remaining.map(function (entry) { var profile = Object.assign({}, customerProfile(entry)); profile.family_relations = (Array.isArray(profile.family_relations) ? profile.family_relations : []).filter(function (relation) { return String(relation.customer_id) !== String(member.id); }); if (remaining.length < 2) { delete profile.family_group_id; profile.family_relations = []; } return { id: entry.id, profile: profile }; });
      var memberProfile = Object.assign({}, customerProfile(member)); delete memberProfile.family_group_id; memberProfile.family_relations = []; updates.push({ id: member.id, profile: memberProfile });
      return saveFamilyProfiles(updates).then(function () { renderContent(); if (typeof window.toast === 'function') window.toast(member.name + ' 고객을 가족에서 제외했습니다.'); });
    }).catch(saveError);
  }
  ...
  function saveEvent() {
    ...
    var timeRow = document.getElementById('iwf-event-timerow'), allDay = document.getElementById('iwf-event-all-day');
    var hasTime = timeRow && !timeRow.hasAttribute('hidden') && !(allDay && allDay.checked);
  ...
  window.OSInsuwork = {
    ...
    setAssetView: function (view) { if (['list', 'thumb', 'large'].indexOf(view) < 0) return; state.assetView = view; localStorage.setItem(canUseAssetActions() ? 'iw_asset_card_view' : 'ws_asset_view', view); renderContent(); },
    ...
    assetMore: assetMore, assetMenuAction: assetMenuAction, assetMoveOpen: assetMoveOpen, assetMoveEnter: assetMoveEnter, assetMoveConfirm: assetMoveConfirm, assetMoveNewFolder: assetMoveNewFolder,
