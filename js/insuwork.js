(function () {
  ...
  var IM_TAESUNG_USER_ID = '98c5f4f9-10c1-4ee1-a656-5c2ca63239fd';
  ...
  var state = {
    section: 'home', assetFilter: 'all', assetView: localStorage.getItem('ws_asset_view') || 'list', assetFolder: null, consultationStatusFilter: 'all', customerStatusFilter: 'all', customerConsultStatusFilter: 'all', query: '', composing: false, searchTimer: 0,
  ...
  function imTaesungPilot() { return String(currentUserId() || '') === IM_TAESUNG_USER_ID; }
  ...
  /* 2026-08-25 대표 확정 — 고객 상세카드 "상담내용" 메모·첨부(saveCustomerRich, workspace_category:'customer')가
  ...
  function latestConsultationForCustomer(customerId) {
    var latest = null;
    state.data.consultations.forEach(function (entry) {
      if (String(entry.customer_id) !== String(customerId)) return;
      if (!latest || String(entry.consulted_at || entry.created_at || '') > String(latest.consulted_at || latest.created_at || '')) latest = entry;
  ...
  function customerConsultStageCounts(rows) {
    var counts = { all: rows.length }; CONSULT_STAGES.forEach(function (stage) { counts[stage.key] = 0; });
    rows.forEach(function (item) { var status = customerConsultationStatus(item); if (counts.hasOwnProperty(status)) counts[status]++; });
    return counts;
  }
  function customersHtml() {
    var pilot = imTaesungPilot();
    var columns = [{ key: 'date', label: '청약일자', width: 86 }, { key: 'name', label: '이름', width: 88 }, { key: 'birth', label: '생년월일', width: 92 }, { key: 'genderAge', label: '성별(보험나이)', width: 104 }, { key: 'phone', label: '전화번호', width: 116 }, { key: 'summary', label: '상담내용', width: 360, flex: true }, { key: 'status', label: '고객상태', width: 102 }];
    if (pilot) columns = [{ key: 'date', label: '청약일자', width: 86 }, { key: 'consultDate', label: '최근상담', width: 86 }, { key: 'name', label: '이름', width: 88 }, { key: 'birth', label: '생년월일', width: 92 }, { key: 'genderAge', label: '성별(보험나이)', width: 104 }, { key: 'phone', label: '전화번호', width: 116 }, { key: 'consultSummary', label: '상담내용', width: 300, flex: true }, { key: 'status', label: '고객상태', width: 102 }];
    ...
    var counts = pilot ? customerConsultStageCounts(baseRows) : customerStageCounts(baseRows);
    var activeFilter = pilot ? state.customerConsultStatusFilter : state.customerStatusFilter;
    var rows = baseRows.filter(function (item) { var profile = customerProfile(item), note = profile.note || '', latestConsult = latestConsultationForCustomer(item.id), consultText = latestConsult ? stripHtml(latestConsult.memo || latestConsult.content || '') : '', status = pilot ? customerConsultationStatus(item) : customerDisplayStatus(item); return (activeFilter === 'all' || status === activeFilter) && matches((item.name || '') + ' ' + (item.phone || item.phone_raw || '') + ' ' + (profile.birth_date || '') + ' ' + note + ' ' + consultText + ' ' + status); });
  ...
  /* 2026-08-25 대표 확정 — 고객 상세카드 "상담내용" 메모 입력창 바로 아래 "+ 일정 추가" 버튼에서 호출.
