(function () {
  ...
  function familySectionHtml(customer) {
    ...
    return '<section class="iw-family-section"><h3>가족</h3><div class="iw-family-members">' + memberRows + '</div>' + candidateHtml + '<div class="iw-family-connect"><select id="iwd-family-customer" aria-label="연결할 기존 고객"><option value="">기존 고객 선택</option>' + options + '</select><select id="iwd-family-relation" aria-label="가족 관계"><option>배우자</option><option>부모</option><option>자녀</option><option>형제자매</option><option>기타</option></select><button type="button" class="iw-btn" onclick="OSInsuwork.connectFamily(\'' + esc(customer.id) + '\')">가족 연결</button></div></section>';
  }
