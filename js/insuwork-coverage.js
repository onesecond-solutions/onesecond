(function () {
  ...
  function normalize(record, preserveLayout) {
    ...
    next.rows.forEach(function (row) {
      ...
      Object.assign(target, row, { id: targetId, hidden: targetHidden, selected: targetSelected, group: targetGroup || row.group, values: combinedValues, sourceDetails: details, valueSources: sources, sourceNames: names });
  ...
  function normalizedBirthDate(value) {
    var text = cellText(value), match = text.match(/(?:^|\D)((?:19|20)\d{2})\s*[.\-/년]\s*(\d{1,2})\s*[.\-/월]\s*(\d{1,2})(?:일|\D|$)/) || text.match(/^((?:19|20)\d{2})(\d{2})(\d{2})$/);
    if (!match) return '';
    var year = +match[1], month = +match[2], day = +match[3], date = new Date(year, month - 1, day);
    if (date.getFullYear() !== year || date.getMonth() !== month - 1 || date.getDate() !== day || date > new Date()) return '';
    return year + '-' + String(month).padStart(2, '0') + '-' + String(day).padStart(2, '0');
  }
  function extractedCustomerInfo(lines, fileName) {
    ...
    if (!info.name) { var namedFile = String(fileName || '').match(new RegExp('^(' + nameChars + '?)(?:님)?[_ ]+보장(?:분석|진단)')); if (namedFile) info.name = namedFile[1]; }
    return info;
  }
  function workbookCustomerInfo(workbook, fileName) {
    var lines = [];
    workbook.SheetNames.forEach(function (name) { var grid = window.XLSX.utils.sheet_to_json(workbook.Sheets[name], { header: 1, defval: '', raw: false }); grid.forEach(function (row) { lines.push(row.map(cellText).join(' ')); }); });
    return extractedCustomerInfo(lines, fileName);
  }
  function mergeCustomerInfo(base, incoming) {
    var next = Object.assign({}, base || {}), info = incoming || {};
    if (next.birthDate && info.birthDate && next.birthDate !== info.birthDate || next.name && info.name && next.name !== info.name && !/[○●*ＯO]/.test(next.name + info.name)) throw new Error('서로 다른 고객 정보가 발견되었습니다. 원본을 확인하고 다른 고객 자료라면 기본 양식으로 초기화한 후 불러와 주세요.');
    if (!next.name && info.name) next.name = info.name;
    if (!next.birthDate && info.birthDate) next.birthDate = info.birthDate;
    return next;
  }
  function parseWorkbook(buffer, fileName) {
    ...
    if (specialized) { specialized.customerInfo = workbookCustomerInfo(workbook, fileName); return specialized; }
  ...
  function recordFromStructured(data, fileName, type) {
    ...
    var record = makeRecord(fileName, type, products, rows), info = data && data.customerInfo || {};
    record.customerInfo = { name: cellText(info.name) || extractedCustomerInfo([], fileName).name || '', birthDate: normalizedBirthDate(info.birthDate) };
    return record;
  }
  ...
  function coverageImageBlob(customerId) {
    ...
    var widths = [110, 100, 270, d.hideTotalColumn ? 0 : 110].concat(products.map(function () { return 158; })), headerHeight = 96, rowHeight = 42, totalWidth = widths.reduce(function (sum, width) { return sum + width; }, 0), canvas = document.createElement('canvas'), scale = 2;
  ...
  var exposed = {
    ...
    toggleTotalColumn: function (customerId) { var d = draft(customerId); d.hideTotalColumn = !d.hideTotalColumn; rerender(customerId); },
...
(function () {
  ...
  function close() {
    ...
    session = null; draggedSection = null; draggedRow = null; if (box) { box.close(); box.remove(); box = null; }
  }
  function open() {
  ...
  function render() {
    ...
    show('<h2 id="iw-ca-template-title">기본 양식 편집</h2><p>각 줄의 ↕ 행 이동을 끌어 한 줄씩 원하는 위치로 옮기세요. 분류명과 담보명은 그대로 유지됩니다. 대분류 옆 ↕ 이동은 분류 전체를 옮깁니다. 변경 내역 확인 후 저장해 주세요.</p>' +
      '<div class="iw-ca-template-scroll"><table><thead><tr><th>대분류</th><th>중분류</th><th>담보명</th><th>순서·추가·삭제</th></tr></thead><tbody>' + session.draft.rows.map(function (r, i) {
        ...
        return '<tr ondragover="OSInsuworkCoverageTemplate.dragOver(event)" ondrop="OSInsuworkCoverageTemplate.dropSection(event,' + i + ')">' + ['section', 'group', 'name'].map(function (key) { return '<td>' + (key === 'section' ? handle : '') + '<textarea rows="1" aria-label="' + ({ section: '대분류', group: '중분류', name: '담보명' }[key]) + ' ' + (i + 1) + '" oninput="OSInsuworkCoverageTemplate.set(' + i + ',\'' + key + '\',this.value)">' + esc(r[key]) + '</textarea></td>'; }).join('') +
          ...
          '<button type="button" class="iw-btn" draggable="true" aria-label="' + (i + 1) + '행 이동" ondragstart="OSInsuworkCoverageTemplate.rowDragStart(event,' + i + ')" ondragend="OSInsuworkCoverageTemplate.dragEnd()">↕ 행 이동</button>' +
  ...
  var draggedSection = null, draggedRow = null;
  window.OSInsuworkCoverageTemplate = {
    rowDragStart: function (event, index) {
      draggedSection = null; draggedRow = null;
      if (!allowed() || !session || session.busy || session.reviewed || !session.draft.rows[index]) { event.preventDefault(); return; }
      draggedRow = session.draft.rows[index].id;
      event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('application/x-insuwork-template-row', String(draggedRow));
    },
    ...
    dragEnd: function () { draggedSection = null; draggedRow = null; },
    dragOver: function (event) { if ((draggedSection || draggedRow !== null) && allowed() && session && !session.busy && !session.reviewed) { event.preventDefault(); event.dataTransfer.dropEffect = 'move'; } },
    dropSection: function (event, index) {
      if (draggedRow !== null) {
        var rowId = draggedRow; draggedRow = null; draggedSection = null;
        if (!allowed() || !session || session.busy || session.reviewed || !session.draft.rows[index]) return;
        event.preventDefault();
        mutate(function (rows) { var from = rows.findIndex(function (r) { return r.id === rowId; }); if (from >= 0 && from !== index) rows.splice(index, 0, rows.splice(from, 1)[0]); });
        return;
      }
