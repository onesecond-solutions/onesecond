(function () {
  ...
  function rerender(customerId) { if (api().rerenderCoverageAnalysis) api().rerenderCoverageAnalysis(customerId); }
  ...
  function importFile(customerId, input) {
    ...
    job.then(function (record) { reset(customerId, record); return api().saveCoverageAnalysis(customerId, record, file); }).then(function () { rerender(customerId); }).catch(function (error) { api().coverageError(error.message || String(error)); }).finally(function () { input.value = ''; });
  }
  ...
  function html(customerId, record) {
  ...
  var exposed = {
    html: html, reset: reset, importFile: importFile,
    ...
    save: function (customerId) { var d = draft(customerId); d.updatedAt = new Date().toISOString(); api().saveCoverageAnalysis(customerId, clone(d)).then(function (saved) { if (saved) reset(customerId, saved); rerender(customerId); }).catch(function (e) { api().coverageError(e.message || String(e)); }); },
