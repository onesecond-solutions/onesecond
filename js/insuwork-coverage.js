(function () {
  ...
  function cancerMiddleGroup(name) {
    ...
    if (!text) return '';
    if (/비급여/.test(text) || /암주요치료비/.test(text)) return '치료비3';
    if (/표적항암|양성자방사선|세기조절방사선|중입자방사선/.test(text)) return '치료비2';
    if (/항암방사선|항암약물|항암치료|암수술|로봇암수술/.test(text)) return '치료비1';
  ...
  function normalize(record) {
    ...
    next.rows = (next.rows || []).map(function (r) { var row = Object.assign({ id: uid('coverage'), section: '', group: '', name: '', recommended: '', status: '', total: '', difference: '', values: {}, hidden: false, selected: false }, r); if (row.section === '암') { if (!row.group) row.group = cancerMiddleGroup(row.name); else if (/^치료비\s*[123]$/.test(row.group)) row.group = row.group.replace(/\s+/g, ''); } return row; });
