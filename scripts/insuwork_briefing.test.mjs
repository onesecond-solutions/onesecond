...
const topic=category=>topics.find(t=>t.category===category);
...
test('sensitive reports do not synthesize legal or medical summaries',()=>{const a=normalize(article('주차장법 개정 안내'),topic('교통·자동차·생활안전'),now);assert.equal(a.description,'');assert.equal(a.verification,'원문 확인 필요');const b=normalize(article('암 치료제 승인'),topics[1],now);assert.equal(b.description,'');assert.equal(b.sourceType,'언론 보도');});
test('official source domain cannot be spoofed by suffix',()=>{assert.equal(normalize(article('건강보험 급여 확대','https://mohw.go.kr/article'),topic('신약·건강보험·의료비'),now).sourceType,'공식기관 자료');assert.equal(normalize(article('건강보험 급여 확대','https://mohw.go.kr.evil.com/article'),topic('신약·건강보험·의료비'),now).sourceType,'언론 보도');});
...
test('balanced first batch across seven topics',()=>{const titles=['실손보험 보장 개편','암 진단 수술 치료','건강보험 급여 확대','기초연금 개편','도로교통법 과태료 시행','소비자 피해 보상 법률 개정','의료비 세액공제 신청'];const rows=topics.flatMap((t,n)=>[0,1].map(i=>({category:t.category,title:`${titles[n]} ${i+1}차 안내`,url:`https://news.example/${n}/${i}`,publishedAt:now.toISOString(),priority:1})));assert.equal(new Set(selectItems(rows).slice(0,7).map(x=>x.category)).size,7);});
test('rejects today low-value investment, book, event and local parking noise',()=>{
  ...
  assert.equal(isUsefulTitle('AI가 앞당긴 암 백신 치료비 낮출 수 있을까 전문가에게 묻다','신약·건강보험·의료비'),false);
});
test('keeps consultation-useful medical, pension and safety changes',()=>{
  assert.equal(isUsefulTitle('폴라이비 블린사이토 건강보험 급여 첫날','신약·건강보험·의료비'),true);
  assert.equal(isUsefulTitle('암 치료제 건강보험 급여 승인과 본인부담 변경','신약·건강보험·의료비'),true);
...
test('requires a concrete customer cost, coverage, eligibility or action impact',()=>{
  assert.equal(isUsefulTitle('암 연구 새로운 가능성 발표','신약·건강보험·의료비'),false);
  assert.equal(isUsefulTitle('암 치료제 건강보험 급여 본인부담 변경','신약·건강보험·의료비'),true);
...
test('keeps one representative per event and caps categories',()=>{
  const base={category:topic('교통·자동차·생활안전').category,publishedAt:now.toISOString(),priority:2};
  const parking=[
