...
test('OCR response schema requests and returns customer identity with existing product and row data',async()=>{
 ...
 const context={Response,Request,atob,console,Deno:{env:{get:()=> 'test-only'},serve:fn=>handler=fn},fetch:async(url,options)=>{if(url.endsWith('/auth/v1/user'))return Response.json({id:'ordinary-user'});requestBody=JSON.parse(options.body);return Response.json({candidates:[{content:{parts:[{text:JSON.stringify(output)}]}}]});}};
