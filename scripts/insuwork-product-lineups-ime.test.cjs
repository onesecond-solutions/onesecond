...
test('lineup IME preserves search field while composition is active and updates only results',async()=>{
 ...
 const root={set innerHTML(v){shellWrites++;assert.equal((v.match(/data-iwpl-results/g)||[]).length,1);assert.ok(!v.includes('class="<div'));},querySelectorAll(){return []},querySelector(sel){return sel==='[data-iwpl-search]'?input:sel==='[data-iwpl-results]'?results:null}};
