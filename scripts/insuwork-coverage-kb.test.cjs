...
test('KB unavailable premium retains policy and rider amounts; unreadable premium still stops',()=>{
 const a=setup(), p=page(1,[[1,'비급여 주사제','비급여 주사제','250만']]);
 p.items=p.items.filter(i=>i.transform[5]!==670);p.items.push(item('보험료미제공',519,670));
 const r=a.parseKbPdf([p],'report.pdf');assert.equal(r.products.length,1);assert.equal(r.products[0].premium,'보험료미제공');assert.equal(r.rows[0].values[r.products[0].id],'250만');
 p.items[p.items.length-1].str='인식실패';assert.throws(()=>a.parseKbPdf([p],'report.pdf'),/1쪽의 보험료/);
});
