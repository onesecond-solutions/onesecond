...
test('five source tabs preserve the legacy basic draft and isolate unsaved edits',async()=>{
  const {ui,saved}=setup();
  assert.equal(ui.workspaceTabId(),'basic');
  const markup=ui.workspaceHtml(base());
  const expected=['기본형','롯데손해보험','KB손해보험','뱅크샐러드','카카오페이'];
  for(const label of expected)assert.ok(markup.includes(label));
  assert.deepEqual(Array.from(markup.matchAll(/role="tab"[^>]*>([^<]+)<\/button>/g),match=>match[1]),expected);
  assert.equal((markup.match(/role="tab"/g)||[]).length,5);
  ui.setRow(key,'r','name','기본형 수정');
  for(const id of ['lotte','kb','banksalad','kakaopay']){
  ...
  assert.equal(saved.lotte.rows[0].name,'lotte 수정');
  ui.switchWorkspaceTab('lotte');assert.match(ui.workspaceHtml(base()),/lotte 수정/);
  ui.switchWorkspaceTab('kb');
...
test('owner gate and keyboard tab navigation',()=>{
  const ordinary=setup(false).ui;assert.doesNotMatch(ordinary.workspaceHtml(base()),/role="tablist"/);for(const id of ['lotte','kb']){ordinary.switchWorkspaceTab(id);assert.equal(ordinary.workspaceTabId(),'basic');}assert.doesNotMatch(ordinary.workspaceHtml(base()),/롯데손해보험/);
  const ui=setup().ui;ui.workspaceTabKeydown({key:'Home',preventDefault(){}});assert.equal(ui.workspaceTabId(),'basic');ui.workspaceTabKeydown({key:'ArrowRight',preventDefault(){}});assert.equal(ui.workspaceTabId(),'lotte');ui.workspaceTabKeydown({key:'ArrowRight',preventDefault(){}});assert.equal(ui.workspaceTabId(),'kb');ui.switchWorkspaceTab('unknown');assert.equal(ui.workspaceTabId(),'kb');
});
test('private persistence matches exact tab and legacy records remain basic on reload',async()=>{
  ...
  for(const id of ['lotte','kb','banksalad','kakaopay']){active=id;assert.equal(app.coverageWorkingItem(),undefined);const r=base();r.rows[0].name=id;await app.saveCoverageWorkspaceDraft(r,null);assert.equal(app.coverageWorkspaceRecord().rows[0].name,id);assert.equal(app.coverageWorkingItem().visibility,'private');assert.equal(app.coverageWorkingItem().owner_id,'owner');}
  active='lotte';assert.equal(app.coverageWorkspaceRecord().rows[0].name,'lotte');assert.equal(app.coverageWorkingItem().legacy_payload.coverage_analysis_tab,'lotte');
