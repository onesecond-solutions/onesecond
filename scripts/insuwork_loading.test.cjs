...
(async () => {
  ...
  careRows.push(...care.careTaskTargets(customer, '2026-01-01').map((t,i)=>({id:'care-'+i,customer_id:'c1',legacy_id:t.legacyId,title:t.title,task_date:t.date,description:t.description})));
  ...
  existing = targets.slice(1).map((t,i)=>({id:'row-'+i,customer_id:'c1',legacy_id:t.legacyId,title:t.title,task_date:t.date,description:t.description}));
