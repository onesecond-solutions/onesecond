...
### 미리보기 금지 원칙 — AI는 앱 미리보기를 화면에 띄우지 않는다 (2026-07-12 대표 확정 · 회귀 판정 기준)

> **AI(총괄·서브에이전트)는 검증을 위해 `app.html`·`design-preview`를 Browser 미리보기 창(preview 패널)에 렌더·표시하지 않는다. 로컬 서버를 띄워 앱 화면을 대표에게 보여주는 행위 자체가 금지다.** (2026-07-11·07-12 반복 지적 — 이번 실수 박제)

- **코드 검증 = 헤드리스/정적으로만** — JS 구문 파싱 확인·diff 정적 리뷰. 미리보기 창에 앱을 띄우는 순간 회귀.
- **시각·동작 확인 = 대표 직접**(본인 Chrome, 임태성 로그인). 로그인·미리보기 대행 안 함.
- **위임 시에도** 서브에이전트가 미리보기를 띄우지 않도록 지시에 명시. (근본 원인 = `os-builder.md` 검증 섹션이 "실시간 렌더"를 지시하던 것 → 2026-07-12 교정)
